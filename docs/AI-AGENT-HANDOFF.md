...
**آخر تحديث:** 2026-06-03 · **آخر متدفوع:** V21.9.234 · **متعمل محلياً (لسه محتاج push):** V21.9.235→237 (UI: تدخّل يدوي + تحليلات + تصعيدات).
**⚠️ بعد الـ push:** deploy `firestore.rules` (قاعدة `aiAgentTakeovers` الجديدة) عشان شارات التدخّل تظهر لحظياً — الأزرار شغّالة من غيره.
...
> **✅ حالة (V21.9.235→237):** A1 (التدخّل اليدوي) **اتعمل** · A2 (المحادثات المحسّنة — فلتر متوقّف/مُصعّد + شارات + تفاصيل + resolve) **اتعمل** · A3 (لوحة التحليلات الحقيقية) **اتعمل**. الباقي: **A4** (اقتراحات/تغذية راجعة) **مؤجّل عن قصد** — الباك-إند لسه مابيـ emit نوع `faq_suggestion`، والـ SuggestionsTab بيتعامل تمام مع `lid_phone_mapping` (النوع الوحيد المُرسَل). يتعمل لما الباك-إند يبدأ يقترح FAQs. الأولوية الجاية = **B5 سقف التكلفة** (مهم لـ Sonnet).
