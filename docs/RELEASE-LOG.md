...
## V21.27.219 (2026-07-01) — ↩️ سلسلة مرتجعات المشتريات: منع حذف الاستلام + عكس الإشعار المدين

**المصدر:** الفحص الشامل — M1 (متوسط) + M6 (متوسط، مرآة C3 على جانب المشتريات).

### M1 — `deleteReceipt` كان بيتجاهل مرتجعات المشتريات
**Root cause:** الحذف كان بيفحص الشيكات المقفولة والفاتورة المرتبطة بس، مش
`r._returns`. فبيعكس كامل الكمية المستلمة ويشيل حركات `receipt` فقط — حركات
`purchase_return` (out) والإشعار المدين يفضلوا → دفتر الحركات سالب (100−30−100=
−30) والمورد متخصم له بضاعة اتحذفت. **الإصلاح (§14.2):** guard جديد يمنع حذف
الاستلام لو عليه `_returns` ويوجّه لحذف/إلغاء الإشعار المدين الأول.

### M6 — إلغاء/حذف الإشعار المدين ماكانش بيزامن التشغيلي (مرآة C3)
**Root cause:** `handleVoid`/`handleDelete` في `DebitNotesPg` كانوا بيعملوا قيد
عكسي بس — المخزون (حركة `purchase_return`) و`receipt._returns` يفضلوا → المورد
يرجع مدين بالكامل بس البضاعة برّه المخزن والكمية محجوزة. **الإصلاح:**
- `reverseDebitNoteReceiptReturns` (جديد في `invoices.js`): بيدوّر على
  `purchaseReceipts[]._returns` بالـ debitNoteId، يرجّع المخزون
  (`applyStockDelta +` + حركة `purchase_return_cancel` in) ويشيل مدخلات
  `_returns` (بتحرّر الكمية المتاحة للمرتجع).
- **وسم الحركة الأمامية:** `saveReceiptReturn` بقى ينشئ الإشعار **أولاً** ويوسم
  حركة `purchase_return` بالـ `debitNoteId` → العكس بيتم بالكمية الأساسية
  الدقيقة (يدعم الوحدة المزدوجة). fallback من `_returns.qty` للمرتجعات القديمة.
- `DebitNotesPg`: الـ handlers بيندهوا العكس قبل الـ void/delete + تحذير واضح.

**الاختبار (§0.2):** build ✓ · 548 اختبار ✓ (+4 M6: عكس المخزون/حركة cancel،
بدون تفعيل مخزن، legacy fallback، no-op للإشعار غير المرتبط) · eslint ✓ ·
emulator 3/3 ✓.

**ملفات:** `src/utils/invoices.js`، `src/pages/PurchasePg.jsx`،
`src/pages/DebitNotesPg.jsx`، `src/utils/__tests__/debitNoteReverse.test.js`
(جديد) + bump.

**ملاحظة:** المرتجع الحر (`FreeSupplierReturnModal` — مش مربوط باستلام) خارج نطاق
M6 (مفيش `receipt._returns` يزامنه)؛ لو محتاج عكس مخزونه عند إلغاء إشعاره نعمله منفصل.

---

## V21.27.218 (2026-07-01) — 🧵 إصلاح جذري C1: خصم مخزون القص كان no-op صامت

**المصدر:** الفحص الشامل — C1 (حرج). **تأكيد Ahmed (+صورة):** وضع المخزن «صارم»
مفعّل من 29-04-2026 — يعني الميزة مفروض شغّالة، وكل أوردر من ساعتها ما خصمش.

**Root cause:** ترانزاكشنات `addOrder`/`replaceOrder`/`delOrder` كانت بتقرا
`factory/config` الخام وتنادي `checkStockAvailability`/`deductStockForOrder`
عليه. بعد V19.52 (stockMovements→split) وV19.57 (fabrics/accessories→partitioned)
الحقول مش في المستند → الفحص بيعدّي دايمًا، الخصم no-op، ومع ذلك
`order._stockDeducted` بيتختم كامل (فيكشن). كمان الفحص المحلي كان على
`configDoc` الخام (نفس المشكلة)، و`tx.set(configRef,nextCfg)` كان بيعيد حقن
`stockMovements:[]` متجاوزًا `stripSplitArrays`.

**الحل (معماري):**
- **`src/utils/orderStockTx.js` (جديد):** `collectStockIds` (اتحاد المطلوب+
  السابق+الفعلي) → `txReadStockDocs` (قراءة `fabricsDocs/{id}`/
  `accessoriesDocs/{id}`/`stockMovementsDays/{today}` جوه الترانزاكشن) → draft
  صغير بتشتغل عليه **نفس الدوال النقية الموجودة** بدون تعديل عقودها →
  `txWriteStockDocs` (كتابة المتغيّر فقط + حركات اليوم). الأوردر بيتكتب في نفس
  الترانزاكشن → «صارم» ذرّي فعلًا، TOCTOU مقفول على مستندات الأصناف نفسها،
  وصفر لمس لـ config (مفيش contention ولا حقن مصفوفات مقصوصة).
- **`orders.js`:** `_stockDeductedActual` — تتبّع المخصوم **فعليًا** (بيتسجّل بس
  لما الصنف موجود واتخصم). + `refundActualStockForOrder` للحذف: بيسترد من
  actual مش من snapshot الـ needed → أوردرات الفترة الوهمية (مختومة «مخصوم»
  من غير خصم) حذفها **مش** بيضخّم المخزون كذبًا.
- **`App.jsx`:** ٣ مسارات لكل عملية: الخصم مقفول → كتابة مباشرة (بلا لمس config)؛
  مفعّل+مُرحّل (الإنتاج) → ترانزاكشن المستندات المنفصلة؛ مفعّل+غير مُرحّل
  (تنصيبات قديمة) → المسار القديم على config زي ما كان. الفحص المحلي بقى على
  `data` المائيّة. `replaceOrder` بيحافظ على `_stockDeductedActual`.

**الاختبار (§0.2) — فعلي على المحاكي:**
- **`npm run test:emu:stock` (جديد — `scripts/emu-order-stock.mjs`): 7/7 ✅** على
  Firestore Emulator بنفس دوال الإنتاج: خصم ذرّي عند الإنشاء (100→80م) + حركتا
  cut في اليوم + ختم actual · رفض النقص ذرّيًا والرصيد ثابت · تعديل delta
  (80→70م) · حذف يسترد بالكامل (→100م) · حذف أوردر وهمي = صفر استرداد.
- 544 اختبار unit (+8 جديدة في `orderStockDeduct.test.js` منها C1 regressions) ·
  build ✓ · eslint ✓ · `test:emu` 3/3 ✓.

**⚠️ ملاحظة تشغيلية مهمة لأحمد:** أرصدة الخامات/الإكسسوارات الحالية **متضخّمة**
بمقدار كل القصّات من 29-04-2026 (ما خصمتش وقتها). الإصلاح بيمنع التكرار من
دلوقتي و**مش بيخصم بأثر رجعي** (قرار §0.1 — الخصم الرجعي محتاج مراجعة يدوية).
المقترح: جرد/تسوية عبر «تعديل رصيد» أو أوردر تسوية، ولو حابب نعمل أداة
`repair-cut-deductions` (dry-run + تأكيد) نناقشها.

**ملفات:** `src/utils/orderStockTx.js` (جديد)، `src/utils/orders.js`،
`src/App.jsx`، `src/utils/__tests__/orderStockDeduct.test.js` (جديد)،
`scripts/emu-order-stock.mjs` (جديد)، `package.json` (سكربت) + bump.
...
**Root cause:** `buildCustomerSummary` (`accountSummary.js`) كان بيطبّق خصم ثابت
`Number(customer.discount) || 0` (افتراضي **0%**) على كل المبيعات، بينما
`statement.js:115-141` بيطبّق خصم **per-delivery** بأسبقية: جلسة `custDisc` →
`delivery.discPct` → `customer.discount` → **10%** افتراضي. فعميل بلا خصم مسجّل
كان الكشف يخصم 10% والملخّص 0% → **رصيدين مختلفين لنفس العميل** في القائمة/
الواتساب/جهات الاتصال/لوحة التحكم مقابل الكشف.

**الإصلاح:** أعدنا كتابة حساب المبيعات/المرتجعات في `buildCustomerSummary` ليجمع
الصافي **per-entry** بنفس `pickDiscPct` بتاع `statement.js` بالحرف. `discPct`
المُرجَع بقى النسبة الفعلية المخلوطة (للعرض في ملخص الواتساب فقط). النتيجة:
الملخّص = الكشف في كل الشاشات.

**تأثير:** أي رصيد عميل كان بيعتمد على الخصم الافتراضي هيتحرّك للقيمة الصحيحة
(المطابقة للكشف). حدّثنا `inventoryValuation.test.js` (fixture عميل بلا خصم:
مستحق 200→180) لأن ده السلوك الصحيح الجديد.

**الاختبار (§0.2):** build ✓ · 536 اختبار ✓ (+2 regression H4: خصم افتراضي 10%،
أسبقية خصم الجلسة؛ +تحديث inventoryValuation) · emulator 3/3 ✓.

**ملفات:** `src/utils/accountSummary.js`، `src/utils/__tests__/accountSummary.test.js`،
`src/utils/accounting/__tests__/inventoryValuation.test.js` + bump.
...
(بيان) بخط أكبر من الطبيعي ومش متناسق.»

**التشخيص:** خلية «بيان» في جدول حركات الخزنة (`TreasuryPg.jsx:3790`) خطها
`FS-1` (أكبر بـ1px بس من الـ badge)، ومع ذلك بتظهر ~ضعف الحجم على الموبايل.
السبب مش في الكومبوننت: **iOS Safari font boosting / text auto-inflation** —
المتصفح بيكبّر خط النصوص العريضة (خلية «بيان» = `width:100%` + سطر واحد طويل في
جدول بيعمل overflow أفقي) لتحسين القراءة، وبيتجاوز حتى القاعدة
`@media(max-width:768px) table{font-size:12px!important}` في index.html (لأن
الـ boosting بيتطبّق في طبقة بعد حساب الـ styles). مفيش `text-size-adjust`
متظبّط في المشروع فالـ boosting شغّال.

**الحل (`index.html`):** أضفنا `html{-webkit-text-size-adjust:100%;
text-size-adjust:100%}` — الإصلاح القياسي (موجود في normalize.css) اللي بيوقف
التكبير التلقائي ويثبّت الخط على حجمه الفعلي، **من غير** ما يلغي pinch-zoom (مش
بنلمس الـ viewport). النتيجة: «بيان» بقى `12px` زي باقي الجدول، متناسق.

**نطاق:** إصلاح عام على مستوى `html` — بيصلّح أي مكان تاني كان بيتكبّر بنفس
السبب (font boosting على جداول/نصوص عريضة). عرض/CSS بحت.

**§0.2:** عرض بحت — معفى من الاختبار السحابي. build ✓.

---

## V21.27.212 (2026-07-01) — 🖼️ إصلاح: لوجو البراند كان مقطوع في الإعدادات

**الطلب (Ahmed + صورتين):** «نزّلت اللوجو العريض بس ظاهر مقطوع جنب اسم البراند
الجديد.» (لوجو HILTY الحقيقي مستطيل عريض ~3.3:1.)

**السبب (`src/pages/SettingsPg.jsx` — BrandsCard):** في V21.27.210 وسّعنا خانة
اللوجو لـ150px واستخدمنا `maxWidth:100%,maxHeight:100%,objectFit:contain`. مع
الـ flex + percentage max-height، الصورة العريضة كانت بتتقاس بالعرض وتتجاوز
ارتفاع الخانة (56px) فتتقصّ بالـ `overflow:hidden` (بيبان شريحة من النص بس).

**الحل:** رجّعنا للـ pattern الموثوق `width:100%;height:100%;objectFit:contain;
display:block` — بيدّي الصورة صندوق محدّد والـ contain بيعمل letterbox داخله →
اللوجو بيظهر **كامل** مهما كانت نسبته (عريض/مربّع)، من غير قص. طُبّق على الـ
swatch (CLARK الافتراضي + البراندات) + مربّع معاينة الإضافة.

**ملاحظة:** بادج الأوردر/الموديل كان سليم (بيستخدم `height` ثابت + `width:auto` +
objectFit:contain، من غير صندوق overflow) فما اتغيّرش.

**§0.2:** عرض/CSS بحت — معفى من الاختبار السحابي. build ✓.

---

## V21.27.211 (2026-07-01) — 🖨️ طباعة: اسم المصنع في سطر واحد (مش راب)

**الطلب (Ahmed + صورة PDF لتسليم مبيعات):** «فتحت لينك تسليم من الموبايل
وبعته PDF — «CLARK ERP System» طلع عامل راب في العنوان. اكتبه بخط أصغر على
سطر واحد.»

**السبب (`src/utils/print.js` — enhancedStyles للهيدر):** `.hdr-brand` كان
`flex:1` (قابل للانكماش) و`.hdr-title` كان `flex-shrink:0; min-width:160px`
(ماينكمشش) — فعنوان المستند الطويل («مبيعات — تسليم #9 — مكتب فور كيدز...»)
بياخد العرض ويضغط منطقة اللوجو+الاسم لحد ما «CLARK ERP System» يلفّ كل كلمة
في سطر (3 سطور).

**الحل:**
- `.hdr-brand`: `flex:1` → `flex-shrink:0` (منطقة اللوجو+الاسم محميّة، مش بتنضغط).
- `.hdr-brand-name`: `17px` → `13px` + `white-space:nowrap` (سطر واحد، أصغر).
- `.hdr-title`: `flex-shrink:0; min-width:160px` → `flex-shrink:1; min-width:0`
  (العنوان هو اللي بيلفّ لو ضاقت المساحة، مش الاسم).
- `.hdr-brand img`: `max-width:90px` → `78px` (توسعة بسيطة للمساحة).

**§0.2:** تعديل طباعة/CSS بحت — **معفى من الاختبار السحابي**. بيتطبّق على كل
تقارير `printPage` (الهيدر مشترك). build ✓.

---

## V21.27.210 (2026-07-01) — 🖼️ لوجو البراند بشكله الطبيعي عريض (مش مربّع)

**الطلب (Ahmed + صورة):** «عاوز اللوجو ياخد شكل الصورة الحقيقية، يكون بالعرض
الكامل ويظهر كبادج للأوردر. اللوجو يكون عريض نفس عرض لوجو كلارك الافتراضي، مش
مربّع.»

**السبب:** الـ swatch/البادجات كانت بأبعاد مربّعة صغيرة (52px في الإعدادات،
14-16px في البادجات) مع `objectFit:contain` → اللوجو العريض بيتضغط في مربّع.

**التنفيذ (عرض بحت):**
- `src/pages/SettingsPg.jsx` (BrandsCard): الـ`swatch` (CLARK الافتراضي +
  البراندات) + مربّع معاينة الإضافة بقوا **landscape 150×52/56** مع
  `maxWidth/maxHeight:100% + objectFit:contain` → اللوجو بشكله الطبيعي عريض.
- `src/pages/ModelsPg.jsx` + `src/pages/DetPg.jsx` (بادج الموديل/الأوردر):
  اللوجو بقى `height` ثابت (16-18px) + `width:auto` + `maxWidth` → بيحافظ على
  أبعاد الصورة الأصلية (عريض لو الصورة عريضة) بدل المربّع.

**§0.2:** تعديل عرض بحت (أحجام CSS) — **معفى من الاختبار السحابي** حسب القاعدة
الجديدة. build ✓.

---

## V21.27.209 (2026-07-01) — 📋 البروتوكول: اختبار سحابي إلزامي قبل النشر (§0.2)

**الطلب (Ahmed):** «ضيف ده في البروتوكول عشان تعمل تيست سحابي كامل لأي داتا
فلو أو فيتشر قبل النشر.»

**اللي اتعمل (CLAUDE.md فقط — بروتوكول):**
- **§0.2 جديد:** «اختبار سحابي كامل قبل النشر — إلزامي لأي data-flow أو فيتشر».
  بيحدّد: إمتى إلزامي (خزنة/مرتبات/محاسبة/فواتير/مخزون/أوامر/migrations/rules/
  async في hot path/أي فيتشر جديدة)، إمتى معفى (عرض بحت)، وإزاي تعمله خطوة خطوة
  (`npm run test:emu` + سكربت موجّه على الـ emulator + `npm run test:rules` لو
  القواعد اتغيّرت)، والقاعدة: **مايتدفعش على main قبل ما الاختبار السحابي يعدّي**.
- **step 1.b في الـ AUTO-WORKFLOW** (أعلى الملف) بيشاور على §0.2.
- **تحديث §0.1 قاعدة #3** (storage.rules cross-service): بدل «ارفض لو مفيش local
  test» بقت «اختبر على الـ emulator الأول (§0.2)، ولو مش ممكن ارفض».
- تحديث سطر «Last updated».

**السياق:** ده تتويج لـ V21.27.207 (بناء الـ emulator) + V21.27.208 (إثباته
فعليًا 3/3). القاعدة القديمة «مفيش test env → ship and hope» **اتلغت رسميًا**
في البروتوكول.

**ملف:** `CLAUDE.md`. صفر تغيير في كود التطبيق. build ✓.

---

## V21.27.208 (2026-07-01) — ✅ تأكيد بيئة الاختبار فعليًا + أمر `test:emu`

**الطلب (Ahmed):** «ممكن انت تعمل الاختبار بنفسك وتشوف النتيجة الفعلية وتصلّح
لو في أخطاء قبل الرفع.»

**اللي اتعمل فعليًا في البيئة السحابية:**
1. `npm run test:rules` → **20 اختبار قواعد عدّت** ضد Firestore emulator (رسائل
   PERMISSION_DENIED دي حالات سلبية بتتأكد إن المنع شغّال).
2. شغّلنا الـ **suite بالكامل** (auth+firestore+storage) بالـ`firebase.json`
   الجديد + سكربت round-trip بيقلّد ربط `src/firebase.js` بالحرف →
   **3/3 نجحت:** Auth(9099) تسجيل+دخول · Firestore(8080) قراءة موثّقة على
   القواعد الحقيقية · Storage(9199) رفع+URL.

**النتيجة:** ربط V21.27.207 **صحيح فعليًا** — مفيش أخطاء تتصلّح. الـ warnings
الوحيدة كانت `EAFNOSUPPORT ::1` (الحاوية السحابية مالهاش IPv6) — غير مؤثرة،
الـ emulator رجع لـ IPv4 واشتغل، وعلى الماك (IPv6 موجود) مبتظهرش أصلاً. **قرار:
مش بنثبّت host على 127.0.0.1** عشان ده ممكن يكسر وصول `localhost:4000` للـ UI
على الماك اللي بيفضّل ::1 — الافتراضي (localhost، بيربط العائلتين) أأمن للماك.

**القيمة المضافة (بدل ما الاختبار يفضل ad-hoc):**
- سكربت دائم `scripts/emu-smoke.mjs` + أمر `npm run test:emu` — بيشغّل الـ
  emulator ويعمل الـ round-trip ويطلّع PASS/FAIL. تأكيد بأمر واحد إن بيئة
  الاختبار سليمة (اتّجرّب: 3/3 ✅). بيستخدم مشروع demo (أوفلاين، صفر إنتاج).
- تحديث `docs/TESTING.md` بالأمر الجديد (طبقة ٣أ).

**ملفات:** `scripts/emu-smoke.mjs` (جديد)، `package.json` (سكربت `test:emu`)،
`docs/TESTING.md`. مفيش أي تغيير في كود التطبيق → صفر تأثير على المستخدمين.
build ✓.
...
اختار البراند، وفي إعدادات الأوردر أدخل أسماء البراندات والصورة. أوردر كلارك
يطلع لوجو كلارك وأوردر هيلتي يطلع لوجو هيلتي. ولما أختار الموديل للتشغيل ياخد
نفس البراند. وفي بطاقة الموديل والأوردر يظهر اللوجو صغير.»

**نموذج البيانات:** `factory/config.brands = [{id,name,logo}]` — مصفوفة إعدادات
صغيرة (زي `garmentTypes`/`sizeSets`)، مش محتاجة splitting (§2). CLARK = الافتراضي
المدمج (فاضي brandId → لوجو المصنع). `data.brands` بيوصل تلقائيًا عبر
`{...config}` في App.jsx:6019 — من غير تعديل هناك.

**الملفات:**
- `src/utils/orders.js`: `brandId:""` في `mkOrder`؛ `buildOrderFromModel`
  بيورّث `model.brandId`؛ `buildModelFromOrder` بيحتفظ بيه. + helper
  `getBrand(data,brandId)` (مصدر حقيقة واحد للكارت/الأوردر/الطباعة).
- `src/pages/SettingsPg.jsx`: مكوّن `BrandsCard` (إضافة/تعديل/حذف براند + رفع
  لوجو بنفس آلية `compressImage` بتاعة لوجو المصنع) — بيتعرض تحت كارت اللوجو.
- `src/pages/ModelForm.jsx`: اختيار «🏷️ البراند» في الهيدر (يظهر لو فيه براندات).
- `src/pages/ModelsPg.jsx`: شارة البراند (لوجو 16px + اسم) على بطاقة الموديل.
- `src/pages/OrdForm.jsx`: اختيار البراند في فورم الأوردر (موروث من الموديل،
  قابل للتغيير).
- `src/pages/DetPg.jsx`: شارة البراند على بطاقة الأوردر + تمرير لوجو البراند
  لطباعة أمر التشغيل عند زر 🖨.
- `src/utils/print-extras.js`: `printOrderSheet(...,configInfo)` — بيمرّر
  `configInfo` (لوجو/اسم البراند) لـ`printPage`. متوافق للخلف: بدون configInfo →
  لوجو CLARK الافتراضي؛ ومع براند → لوجو البراند.

**backward-compatible بالكامل:** كل الموديلات/الأوامر القديمة `brandId` فاضي →
سلوك CLARK الحالي بالضبط. الميزة كلها additive/presentational (مفيش mutation
مالي ولا async patterns جديدة). build ✓ · 531 اختبار · eslint نظيف.

**ملاحظة:** التغيير في طباعة أمر التشغيل بيخلّي الأوردر بدون براند يستخدم لوجو
المصنع (`data.logo`) بدل لوجو CLARK الجينيري لو المصنع رافع لوجو — ده اتّساق مع
باقي التقارير (اللي كلها بتمرّر `data.logo`)، ولو مفيش لوجو مرفوع بيرجع CLARK.

---

## V21.27.205 (2026-07-01) — ⏳ بورتال العميل: مؤشر «جاري تحميل الموديلات المتاحة»

**الطلب (Ahmed):** «في بورتال العميل، لما يضغط تاب اطلب عاوز يظهر ‹جاري
تحميل الموديلات المتاحة› عشان بياخد وقت شوية لحد ما يعرض الصور ويختفي.»

**المشكلة:** حالة `loading` القديمة كانت بتتلغى بمجرّد رجوع الـ API
(`/api/customer-portal-catalog`)، فالجريد كان بيظهر والصور لسه بتتحمّل رمادي
قدّام العميل (الصور `loading="lazy"` بتتجاب بعد الـ JSON).

**التنفيذ (`src/components/portal/CustomerOrderTab.jsx`):**
- state جديدة `imgsReady` + effect بيعمل **preload لأول ~12 موديل** (فوق الطية)
  بـ `new Image()` → الصور تدخل كاش المتصفح، فلما الجريد يترسم تبان فوراً.
- المؤشر «جاري تحميل الموديلات المتاحة» (سبينر CSS + نص) يفضل ظاهر طول
  `loading || !imgsReady`. أمان `setTimeout 6s` عشان مايعلّقش لو صورة بطيئة/بايظة.
- كاش موديول `imgsPreloaded[custId]` → التنقّل بين تابات البورتال مايعيدش المؤشر.
- الباقي (موديلات ما بعد أول شاشة) بيتحمّل lazy مع السكرول زي ما هو.

نقلنا فحص `error` قبل السبينر عشان خطأ التحميل يبان مايتغطّاش. build ✓.

---

## V21.27.204 (2026-07-01) — 🖨️ كشف الحساب: عناوين متكرّرة كل صفحة + هيدر/إجماليات أسود

**الطلب (Ahmed):** «في الطباعة لما يكون أكتر من صفحة، صف العناوين ينزل مع كل
صفحة؟ (مبيعات ومشتريات) — وعاوز صف العناوين أسود/كتابة بيضا، وصف الإجماليات
أسود/كتابة بيضا.»

**التنفيذ (`src/components/AccountStatementView.jsx` — دالة `doPrint`):**

1. **تكرار صف العناوين كل صفحة:** الجدول أصلاً مبني بـ `<thead>`/`<tbody>`/
   `<tfoot>` صحيح، فالمتصفح بيعيد الـ `<thead>` أعلى كل صفحة تلقائيًا عند
   الطباعة (`display:table-header-group` هو الافتراضي). أكّدناه صراحةً في
   `<style>` مضمّن + `page-break-inside:avoid` على الصفوف.

2. **هيدر + إجماليات أسود/أبيض:** غيّرنا خلفية صف العناوين من `accent` (أزرق/
   برتقالي) وصف الإجماليات من `#eff6ff` → **أسود `#000` + كتابة بيضا `#fff`**.

**العقبة المعمارية (مهمة):** `PRINT_CSS` فيه قاعدة عامة
`…table *{color:#000!important}` بتفرض كل نص الجدول أسود — فالـ `color:#fff`
inline كان بيتلغى في الطباعة. الحل: قاعدة CSS مضمّنة أعلى تخصيصًا
(`#report-content table.stmt-table thead th{color:#fff!important}` — تخصيص
`id+class+type` يكسب `#report-content *`) + `print-color-adjust:exact` عشان
الخلفية السودا تتطبع فعليًا (المتصفح بيشيل الخلفيات في الطباعة افتراضيًا).
أضفنا `class="stmt-table"` على الجدول عشان القاعدة تستهدفه.

**نطاق:** نفس المكوّن بيخدم العميل والمورد → غطّى المبيعات والمشتريات معًا.

**قيد معروف:** التكرار شغّال في طباعة المتصفح (🖨 طباعة). «حفظ PDF»
(html2pdf/html2canvas) بيصوّر التقرير كصورة ويقسّمها، فالعنوان بيظهر مرة
واحدة — دي طبيعة الـ raster، مش bug.

build ✓ · 531 اختبار.

---

## V21.27.203 (2026-07-01) — ✂️ كشف الحساب: إزالة كلمة «التفقيط:»

**الطلب (Ahmed):** «من التقرير امسح كلمة التفقيط، ومن المشتريات كمان.»

**التنفيذ (`src/components/AccountStatementView.jsx`):** شِلنا الـ `<span>التفقيط:
</span>` من سطر التفقيط — بقى يعرض نص `tafqitEGP` لوحده («فقط … لا غير»). نفس
المكوّن بيخدم العميل والمورد فالتغيير غطّى الكشفين. build ✓ · 531 اختبار.

---

## V21.27.202 (2026-07-01) — 🔢 تفقيط عربي في كشف حساب العميل والمورد

**الطلب (Ahmed):** «من هب المبيعات ← كشف حساب ← تقرير كشف حساب العميل، عاوز
تفقيط للأرقام بالعربي بشكل احترافي، وكشف حساب المورد كمان.»

**التنفيذ (`src/components/AccountStatementView.jsx`):** طباعة كشف الحساب
(المكوّن الواحد اللي بيخدم العميل والمورد عبر `partyType`) بقت تعرض سطر
**التفقيط** تحت الإجمالي (`result.totals.closing`) مباشرة، باستخدام
`tafqitEGP` (الدالة النقية الموجودة، متغطّاة بـ14 اختبار). مثال:
«فقط مائتان وثلاثة وعشرون ألف وتسعمائة واثنان وثمانون جنيهاً مصرياً لا غير»
— وبيتعامل مع القروش (…وخمسون قرشاً…).

build ✓ · 531 اختبار.

> ملاحظة: ده بيغطّي كشف حساب المبيعات/المشتريات (AccountStatementView). لو
> Ahmed عايز التفقيط في دفتر الأستاذ المحاسبي (`reportPrint.js`) كمان، نضيفه.

---

## V21.27.201 (2026-07-01) — 🗂️ الملفات اليتيمة ما تختفيش + تعديل الجهات المرتبطة

**الطلبات (Ahmed):** «(1) عملت تحميل لفولدر كامل، ظهر يوم كامل واختفى دلوقتي مع
إنه موجود في المساحة الإجمالية — نحلها جذريًا. (2) في جهات الاتصال عاوز أقدر أعدّل.»

### 1) اختفاء الملفات — root cause + إصلاح دفاعي
**Root cause:** بعد V178 كل ملف بقى مستند `factory/df_<id>`، بس **المجلدات** لسه في
مستند واحد `factory/documentsTree` بيتحفظ بـ `setDoc(merge:false)` = **last-write-wins**.
جهاز بنسخة قديمة من الشجرة (u4 listener ما لحقش يتحدّث) بيعمل أي `upDocs` →
بيكتب `documentsTree` من غير المجلد اللي جهاز تاني ضافه → **المجلد يتفقد**،
وملفاته (df_ docs سليمة) تبقى **يتيمة** (`folderId` لمجلد مش موجود). و`DocumentsPg`
كان بيفلتر `(f.folderId||null)===currentFolderId` → اليتيمة تختفي من كل مكان
(بايتسها فاضلة في Storage — «موجودة في المساحة الإجمالية»). ده نفس anti-pattern
§10 (مصفوفة في مستند واحد من أجهزة متعددة).

**الإصلاح الدفاعي (`src/pages/DocumentsPg.jsx`) — آمن، قراءة فقط:** الملفات
اللي `folderId` بتاعها مش في مجلدات الشجرة تُعتبر **في الجذر** فتظهر بدل ما
تختفي. صفر فقدان بيانات من العرض؛ المستخدم يقدر يرجّعها لمجلد.

> **🔜 الحل الجذري (محتاج مناقشة قبل التنفيذ — §0.1):** نقل المجلدات لمستندات
> مستقلة (`factory/dfolder_<id>`) زي ما عملنا للملفات في V178 — بيلغي الـ
> single-doc overwrite تمامًا. migration + listener + merge، blast radius عالي،
> مفيش local test → أقترحه ومنفّذوش أعمى.

### 2) تعديل جهات الاتصال المرتبطة (`src/pages/ContactsPg.jsx`)
كان `handleEditSave` + `ContactDetailModal.handleSave` بيرفضوا التعديل لأي جهة
`linkedFrom !== "contact"` (رسالة «استخدم الصفحة الأصلية»). دلوقتي:
- المودال بيعرض فورم تعديل (اسم + تليفون) للجهات المرتبطة كمان.
- `handleEditSave` للجهة المرتبطة بيكتب الاسم/التليفون في السجل الأصلي عبر
  `entityIds` (`customers/suppliers/workshops/employees`) بـ `upConfig`
  (بيزامن الـ partitioned). لو الجهة مرتبطة بأكتر من نوع، بيحدّثهم كلهم.

build ✓ · 531 اختبار · (التخزين: قراءة فقط؛ الجهات: كتابة على السجل الأصلي).

---

## V21.27.200 (2026-07-01) — 📱 إصلاح زوم الموبايل المزعج عند الكتابة في الحقول

**الطلب (Ahmed):** «في أمر بيع جديد على الموبايل، لما أضغط حقل الصنف أو أي حقل
كتابة بيكبر بشكل مايبقاش ينفع أكتب/أختار وبيفضل عامل زوم. أكيد بيحصل في كل شاشات
الإدخال — ممكن نصلحه.»

**Root cause:** سلوك iOS Safari الكلاسيكي — بيعمل **auto-zoom** على أي
`input/select/textarea` **خطه < 16px** عند الـ focus، وبيفضل مزوّم. في
`index.html` كانت فيه قاعدة داخل `@media(max-width:768px)`:
`input,select{font-size:13px!important}` → 13px < 16px → زوم على كل حقل في
الموبايل.

**الحل (`index.html`):** غيّرنا القاعدة لـ
`input,select,textarea{font-size:16px!important}` — 16px هو الحد اللي iOS
بيوقف عنده الزوم. حل **عام** (بيغطّي كل حقول كل الشاشات على الموبايل دفعة
واحدة) ومتوافق مع الـ accessibility (مش بنلغي الـ pinch-zoom من الـ viewport،
بنرفع خط الحقول بس).

> ليه مش `user-scalable=no`؟ لأنه بيلغي التكبير اليدوي (accessibility) و iOS
> بيتجاهله أصلاً في الإصدارات الحديثة. رفع الخط لـ16px هو الحل الموصى به.

build ✓ · 531 اختبار · CSS فقط.

---

## V21.27.199 (2026-07-01) — 📏 تقييم المخزون: الوحدة جنب الكمية

**الطلب (Ahmed):** «في لوحة التحكم، لما بضغط تقييم مخزون عاوز الوحدة تظهر جنب
الكمية مباشرة.» (الصورة: عمود الكمية بيعرض 1,273 من غير «متر».)

**الحل:**
- **`src/utils/dashboardKpis.js`:** أضفنا حقل `unit` لكل صفوف التقييم —
  `fabricDetail` (`f.unit||"متر"`)، `accessoryDetail` (`a.unit||"قطعة"`)،
  أصناف المخازن المخصّصة (`it.unit` حسب الفئة)، و`finishedDetail`
  (map → `unit:"قطعة"`).
- **`src/components/DashboardKpis.jsx`:** عمود الكمية في `invCols` بقى له
  `unitKey:"unit"`، ومسار العرض (React + الطباعة/PDF) بيلحق الوحدة بعد الرقم لو
  موجودة: `fmt(qty) + " " + row.unit`. الأعمدة التانية مش متأثرة.

النتيجة: «1,273 متر» / «15 كيلو» / «50 قطعة» بدل الرقم لوحده. build ✓ ·
531 اختبار (تغيير عرض — الحسابات والإجماليات ثابتة).

---

## V21.27.198 (2026-06-30) — 🔢 لوحة التحكم: أرقام بدون كسور عشرية

**الطلب (Ahmed):** «في لوحة التحكم عاوز الأرقام صحيحة بدون علامات عشرية.»
(الصورة: صافي الربح 2,004,583.58 · إجمالي تقييم المخزون 4,162,034.4 · تقييم
الإكسسوار 188,984.72 · تقييم الجاهز 3,643,570.68 — كلها بكسور.)

**الحل (`src/components/DashboardKpis.jsx`):** استبدلنا `fmt` بـ **`fmt0`**
(الموجودة في `format.js` — «always rounds to whole number. Use for money») في
كل مواضع عرض الفلوس: بطاقة الـ KPI (`Card` value)، helper `money()` (خلايا
البوب-أب)، صف الإجمالي في الطباعة/البوب-أب. الأعمدة غير المالية (`c.money===false`
— كميات) بتفضل `fmt` زي ما هي. صفر تغيير في الحسابات (`dashboardKpis.js`) — عرض فقط.

build ✓ · 531 اختبار.

---

## V21.27.197 (2026-06-30) — 📇 جهات الاتصال (موبايل): الجدول يتسع كامل

**الطلب (Ahmed):** «بالفلتر الشاشة بتظهر كلها بدون سكرول كويس؛ لما بلغيه بتظهر
ناقصة. مفيش داعي للسكرول — ممكن تظهر كلها زي حالة الفلتر.»

**Root cause (`src/pages/ContactsPg.jsx`):** جدول جهات الاتصال على الموبايل
`minWidth:auto` + `tableLayout:auto` (الافتراضي) → عرضه = عرض **أوسع صف** في
الـ1673 جهة. مع إضافة زرّي الاتصال/الواتساب (V195/V196) + الأرقام الطويلة +
أسماء طويلة/تصنيفات متعددة، أوسع صف بيتعدّى عرض الشاشة، والحاوية
(`overflowX:hidden` على الموبايل) بتقصّ من الشمال (RTL) → زر الاتصال + عمود
التصنيفات بيختفوا. بالفلتر (صف واحد ضيّق) الجدول بيتسع فيبان كامل.

**الحل (موبايل فقط):**
- الجدول `tableLayout:fixed` + `width:100%` → **مستحيل يتعدّى عرض الشاشة**؛ أي
  محتوى أطول بيتقصّ **داخل عموده** (مش بيوسّع الجدول ولا يتقطّع بره الشاشة).
- أعمدة بعرض ثابت: التليفون 160px، التصنيفات 52px، الاسم الباقي.
- الاسم + الرقم: `overflow:hidden + textOverflow:ellipsis + nowrap` (يتقصّروا
  بـ«…» للطويل).
- أزرار الاتصال/الواتساب 32→28px على الموبايل + خط الرقم FS-2 — عشان كله يتسع.

النتيجة: كل الصفوف تظهر كاملة (اتصال + واتساب + رقم + تصنيف) بدون قص ولا سكرول
أفقي. الديسكتوب (`tableLayout:auto`) لم يتغيّر.

build ✓ · 531 اختبار · تخطيط/CSS فقط.

---

## V21.27.196 (2026-06-30) — 💬 زر واتساب (موبايل) + هبّين في هوم الموبايل

**الطلب (Ahmed):** «ضيف زر واتساب لنسخة الموبايل بس. وممكن نضيف هب جهات الاتصال
في هوم الموبايل + هب الحملات والرسايل.»

### 1) زر واتساب — موبايل فقط (`src/pages/ContactsPg.jsx`)
جنب زر الاتصال (V195)، زر **واتساب أخضر** (`#25D366` + جليف SVG رسمي)
`<a href="https://wa.me/<digits>" target="_blank">` — بيفتح محادثة على الرقم.
الرقم من `normalizePhone(c.phone).replace(/[^0-9]/g,"")` (الشكل القانوني المصري
`201...`). **محصور بـ `isMob`** (موبايل فقط). `onClick=stopPropagation`.

### 2) هبّين جداد في هوم الموبايل (`src/pages/MobileHomePage.jsx`)
أضفنا لـ `HOME_BUTTONS` + `ICONS`:
- **«جهات الاتصال»** (`contacts`) — أيقونة أشخاص SVG.
- **«الحملات والرسائل»** (`campaigns`) — أيقونة مكبّر صوت SVG.
`onNavigate=goTo` بيوصّلهم لتاباتهم (محروسين بـ `canViewTab`). الشبكة بقت ١٠
أزرار (3+3+3+1).

build ✓ · 531 اختبار · إضافات UI (مفيش لمس بيانات).

---

## V21.27.195 (2026-06-30) — 📞 زر اتصال مباشر في جهات الاتصال

**الطلب (Ahmed):** «في جهات الاتصال على الموبايل — نضيف جنب كل صف زر اتصال
يفتح على الرقم ويتصل مباشر؟»

**التنفيذ (`src/pages/ContactsPg.jsx`):** في خلية التليفون لكل صف، زر **اتصال
أخضر دائري** (أيقونة سماعة SVG) `<a href="tel:<digits>">` — بيفتح المتصل على
الرقم مباشرة على الموبايل. `onClick=stopPropagation` عشان الزر ما يفتحش
ContactDetailModal (الصف نفسه لسه بيفتح التفاصيل عند الضغط في أي مكان تاني).
الرقم بيتنضّف `replace(/[^\d+]/g,"")`. الزر بيظهر بس لو `c.phone` موجود.
(بيظهر على الموبايل والديسكتوب — `tel:` آمن على الاتنين.)

build ✓ · 531 اختبار · إضافة UI بسيطة (مفيش لمس بيانات).

---

## V21.27.194 (2026-06-30) — 🧵 إصلاح قص جدول الخامات في أوردر التشغيل (موبايل)

**الطلب (Ahmed):** «في نسخة الموبايل، أوردر التشغيل — عاوز يظهر كل المحتوى؛ في
الخامات حاجات ناقصة مش ظاهرة.» (الصورة: جدول ألوان الخامة متقطّع من الشمال —
عمود «الكمية» والشِبات راقات/قطع مش باينين.)

**Root cause:** كرت الخامة في DetPg متحط في `grid` بـ `1fr`. الـ grid item
الافتراضي `min-width:auto` → مبيصغّرش تحت العرض الجوهري للمحتوى. جدول `FCTable`
جوّاه `minWidth:450`، فالكرت بيتمدّد لـ450 ويتعدّى عرض شاشة الموبايل، والحاوية
الرئيسية (`overflowX:hidden` على الموبايل) بتقصّ الزيادة من الشمال (RTL) →
عمود «الكمية» والشِبات بيختفوا.

**الحل:**
- **`src/pages/DetPg.jsx`:** أضفنا `minWidth:0` على كرت الخامة (grid item) →
  بيتقيّد بعرض الشاشة، فالـ `overflowX:auto` الموجود جوّه `FCTable` بقى يشتغل
  فعلاً (تمرير أفقي للجدول داخل الكرت) والشِبات (flexWrap) تلتف بدل ما تتقصّ.
- **`src/components/ui.jsx` (`FCTable`):** في وضع العرض (`readOnly` — أرقام فقط)
  صغّرنا عرض الأعمدة + `minWidth` الجدول (450→300): اللون 160→108، الراقات
  100→52، القطع/راق 120→64، الكمية 80→60 (+توسيط الأرقام). فالجدول بقى يظهر
  كامل من غير تمرير على معظم شاشات الموبايل. وضع التحرير (فيه inputs) لم يتغيّر.

build ✓ · 531 اختبار · تخطيط/CSS فقط.

---

## V21.27.193 (2026-06-30) — 📱 هوم الموبايل: ٣ أزرار في الصف

**الطلب (Ahmed):** «في هوم الموبايل عاوز ٣ أزرار في الصف الواحد مع التنسيق الاحترافي.»

**التغيير (`src/pages/MobileHomePage.jsx`):**
- الشبكة `gridTemplateColumns` من `1fr 1fr` (عمودين) لـ **`repeat(3, 1fr)`** (٣ أعمدة)،
  `gap` 12→10.
- تصغير متناسب يحافظ على التنسيق: الكارت padding `20px 12px`→`15px 6px`،
  `minHeight` 128→112، radius 20→18؛ بلاطة الأيقونة 62→**50** (radius 18→15)؛
  أيقونة SVG default 28→**25**؛ الخط `FS+1`→`FS-1` (`justify-content` بقى
  `flex-start` + `flexShrink:0` للبلاطة عشان الصفوف تفضل متّسقة مع اختلاف طول
  النص). الـ8 أزرار بقت 3+3+2.

build ✓ · 531 اختبار · تخطيط فقط.

---

## V21.27.192 (2026-06-30) — 📱 إعادة تصميم الشريط السفلي + زر الهوم في النص

**الطلب (Ahmed):** «غيّر الشريط السفلي بشكل احترافي، وزر الـ«+» اللي في النص
يبقى زر الهوم، بأيقونات SVG زرقا انسيابية زي المرجع.»

### التغييرات
- **`src/components/navigation/BottomNavFab.jsx`** — كان زر «+» برتقالي يفتح
  قائمة إجراءات سريعة؛ بقى **زر الهوم** المركزي المرتفع (أزرق متدرّج
  `#2E7BED→#1D4ED8` + أيقونة بيت SVG + ring أبيض + ظل أزرق). prop بقى `onHome`.
- **`src/components/navigation/BottomNav.jsx`** — أيقونات **SVG خطّية موحّدة**
  (sales/details/finance/more + home للـ minimal) بدل الإيموجي. تاب نشط بلون
  أزرق `#2563EB` + خلفية **pill أزرق شفّاف** بدل شريط المؤشر. زجاجي أبيض، ظل
  ناعم، انسيابي. الـ minimal home button بقى أيقونة SVG (بدل 🏠) بنفس شكل الـ FAB.
- **`src/utils/navigationConfig.js`** — `BOTTOM_TABS`: شِلنا `home` (بقى زر النص)
  وأضفنا **`details` (التصنيع)** → ٤ تابات متماثلة (sales/details/finance/more).
  `bottomTabFromTabKey` بيرجّع "details" لتمييزه. شِلنا details من قائمة «المزيد»
  (تفادي تكرار).
- **`src/App.jsx`** — `visibleBottomTabs` + `onBottomTabChange` بيتعاملوا مع
  «details» كتاب مباشر. `onFabAction` (قائمة إجراءات الـ FAB) اتشال. زر النص
  بينادي `goHome`.

**ملاحظة:** قائمة الإجراءات السريعة القديمة (فاتورة/خزنة/جهة اتصال/مهمة/إشعار)
اللي كانت في الـ «+» اتشالت — الإجراءات لسه متاحة من صفحاتها + من شبكة الهوم
(QR/خزنة سريعة). لو Ahmed عايزها ترجع في مكان تاني نعملها.

build ✓ · 531 اختبار · تغيير تنقّل/تصميم (مفيش لمس بيانات).

---

## V21.27.191 (2026-06-30) — 📱 إعادة تصميم احترافية لشاشة الموبايل

**الطلب (Ahmed):** «شكل شاشة الموبايل مش كويس — عاوز إعادة تصميم احترافية أبيض
وأزرق شفّاف زي المرجع اللي رفعته.» (مرجع = تطبيق دليل بمظهر أبيض/أزرق نظيف
بأيقونات خطّية موحّدة وكروت ناعمة.)

**Root cause للمظهر «غير الاحترافي» (`src/pages/MobileHomePage.jsx`):**
- إيموجي مختلطة الأنماط (📷 📊 💵 🛒 ✂️ 🧾 🛍️ 🪄) كأيقونات → غير متّسقة.
- خلفيات أيقونة بألوان باستيل عشوائية لكل قسم → مشتّتة.
- شريط ترحيب باهت.

**إعادة التصميم:**
- **أيقونات SVG خطّية موحّدة** (24×24، stroke currentColor، خط 1.9) لكل قسم —
  `dashboard/qr/cart/wallet/receipt/scissors/ai/bag`. مظهر line-icon احترافي
  زي المرجع.
- **هيدر hero أزرق متدرّج** (`#1D4ED8→#2E7BED→#38BDF8`) + لمستين زخرفيتين
  (radial glow) + اسم المستخدم بارز + التاريخ — بدل الشريط القديم.
- **كروت بيضا** (radius 20، border `#EAF0F8`، ظل ناعم مزدوج) + **خلفية أيقونة
  «أزرق شفّاف» موحّدة** (`linear-gradient(#F0F6FF→#DCEAFF)` + border `#CBDEFB`) —
  بدل الألوان العشوائية. press-state (scale + ظل أخف).
- لوحة الألوان: أبيض + أزرق (`BLUE=#2563EB`).

**صفر تغيير في الوظائف/الصلاحيات** — نفس الأزرار والـ `canViewTab` filter
والـ onNavigate/onSpecialAction. تحسين بصري بحت. build ✓ · 531 اختبار.

> ملاحظة: الـ top-bar (الحساب/الموسم/الإشعارات/اللوجو) مكوّن منفصل — لو Ahmed
> عايز يتظبط كمان نعمله في نسخة تانية.

---

## V21.27.190 (2026-06-30) — 🖨 تقرير الأصول الثابتة (أبيض/أسود) + «عرض التقرير» للمبيعات

**الطلبات (Ahmed):** «(1) في الأصول الثابتة عاوز التقرير قابل للطباعة بتنسيق
أبيض/أسود احترافي. (2) في تقرير المبيعات غيّر زر الطباعة لـ«عرض» يفتح صفحة
احترافية أقدر أطبعها أو أعملها PDF.»

### تحسين محرّك الطباعة (`src/utils/print.js`)
`printPage(title, body, configInfo, opts)` — أضفنا الـ4th param `opts`:
- `opts.viewOnly` = يفتح صفحة التقرير **من غير طباعة أوتوماتيك** (المستخدم يختار
  🖨/📄 من الشريط). كان بيطبع تلقائي دايمًا (سطر `pw.print()`).
- `opts.mono` = تنسيق **أبيض/أسود** بالكامل — استبدلنا اللمسات الزرقا في الهيدر
  (border/title box/foot-brand) بأسود/رمادي عبر متغيّرات. الجدول أصلاً رمادي/أسود
  من `PRINT_CSS`.
- **متوافق رجعيًا:** النداء بـ3 args (كل التقارير القديمة) = نفس السلوك بالظبط.

### الأصول الثابتة (`src/pages/FixedAssetsPg.jsx`)
- دالة `printAssetsReport()` تبني جدول HTML (اسم الأصل · الفئة · تكلفة الشراء ·
  قيمة الخردة · الحالة + صف إجمالي من `reportTotals`) وتفتحه بـ
  `printPage(..., {viewOnly:true, mono:true})`.
- زر **«👁 عرض / طباعة التقرير»** في رأس تاب التقارير.

### تقرير المبيعات (`src/pages/CustDeliverPg.jsx`)
- `printSalesReport` بقى ينادي `printPage(..., {viewOnly:true})` (مش طباعة فورية).
- الزر **«🖨 طباعة التقرير» → «👁 عرض التقرير»**.

build ✓ · 531 اختبار ناجح.

---

## V21.27.189 (2026-06-30) — 📐 توسعة أكبر لقائمة العملاء (متابعة V188)

**الطلب (Ahmed):** «ممكن توسّع كمان شوية تظهر مش مقطوعة كده.» — بعد V188 الأزرار
ظهرت بس فضلت متزنوقة والزر الأخير (❌ حذف، danger ببخلفية حمرا فاتحة) باهت/متقطّع
على حافة العمود.

**الحل (`src/pages/CustDeliverPg.jsx`):**
- `cAct`: **220 → 268px** + `paddingInlineEnd` 6→8.
- حاوية الـ view `maxWidth`: **1360 → 1480**.
- المسافة بين أزرار الأدوات (المسار الـ virtualized): `gap` **3 → 5**.

النتيجة: الأدوات الخمسة واضحة ومتباعدة مش متقطّعة. build ✓ · 531 اختبار · تخطيط فقط.

---

## V21.27.188 (2026-06-30) — 📐 توسعة قائمة العملاء (كل الأدوات على الصف)

**الطلب (Ahmed):** «في هب المبيعات ← قائمة العملاء — عاوز توسعة للعرض شوية عشان
كل الأدوات تظهر على الصف.»

**Root cause (`src/pages/CustDeliverPg.jsx`):** قائمة العملاء للقوائم الكبيرة
(>120 عميل → 1558 في حالة Ahmed) بتستخدم المسار الـ **virtualized** (react-window).
عمود الأدوات فيه كان عرضه ثابت **176px** بينما فيه **5 أزرار** (📋 سجل مبيعات ·
✏️ تعديل · QR · 📱 رابط العميل · ❌ حذف) مجموع عرضها ≈ 206px → الزر الأخير
(الحذف) بيتقطع/يتخفي. (الحاوية كانت كمان مضيّقة لـ1200 من V21.27.75.)

**الحل:**
- `cAct` (عمود الأدوات في الصف الـ virtualized): **176 → 220px** — يسع الـ5 أزرار.
  الهيدر بيستخدم نفس `cAct` فالمحاذاة تفضل مظبوطة.
- حاوية الـ view: `maxWidth` **1200 → 1360** (توسعة «شوية» زي الطلب).
- المسار غير الـ virtualized (≤120 عميل) كان أصلاً سليم (`<td>` بعرض تلقائي).

build ✓ · 531 اختبار ناجح · تعديل CSS/تخطيط فقط.

---

## V21.27.187 (2026-06-30) — 🏭 الأصول الثابتة: كارت قيمة الخردة + تقرير

**الطلب (Ahmed):** «في هب الأصول الثابتة — عايز بطاقة إجمالي قيمة الخردة جنب
إجمالي التكلفة، وتقرير بأعمدة: اسم الأصل · الفئة · تكلفة الشراء · قيمة الخردة ·
الحالة.»

**ملاحظة:** حقل `salvageValue` (قيمة الخردة) **موجود بالفعل** في فورم الأصل
(`FixedAssetEditModal`)، فده تعديل **عرض/تقرير فقط** — صفر تغيير في بيانات أو
حسابات مالية.

### التغييرات (`src/pages/FixedAssetsPg.jsx`)
- **كارت «إجمالي قيمة الخردة»** بين «إجمالي التكلفة» و«الإهلاك المتراكم».
  `stats.totalSalvage` = مجموع `salvageValue` للأصول غير المتصرّف فيها (نفس
  قاعدة `totalCost` فيكونوا قابلين للمقارنة). الشبكة بقت `repeat(5,1fr)`
  ديسكتوب / `repeat(2,1fr)` موبايل.
- **تقرير جديد** أعلى تاب «التقارير» بالأعمدة المطلوبة بالظبط: اسم الأصل ·
  الفئة · تكلفة الشراء · قيمة الخردة · الحالة (بادج ملوّن) — + صف إجمالي
  (`reportTotals`: إجمالي التكلفة + إجمالي الخردة لكل الأصول المعروضة، شاملة
  المتصرّف فيها مطابقةً للصفوف). «📊 سجل الأصول الكامل» القديم باقٍ تحته.

build ✓ · 531 اختبار ناجح · تعديل UI فقط (مفيش اختبارات لازمة للصفحة).

---

## V21.27.186 (2026-06-30) — 🔧 تفعيل الإصلاح التلقائي + زر «إصلاح الآن»

**السياق:** بعد ما V21.27.185 بنى آلية الإصلاح التلقائي **معطّلة افتراضيًا**
(لأمان الكتابة المالية غير المُشرَف عليها)، Ahmed طلب تفعيلها صراحةً بعد شرح
المخاطر والمثال. ده الـ informed consent المطلوب في §0.1.

### التغييرات
- **`api/cron/reconcile-financials.js`:** الـ gating اتقلب لـ **on-by-default** —
  `autoRepairOn = (cfg.reconcile||{}).autoRepair !== false`. يعني الإصلاح الليلي
  لأرجل التحويلات المؤكدة الناقصة بقى شغّال تلقائي؛ يتقفل بس بضبط
  `cfg.reconcile.autoRepair=false`. كل حواجز V185 ثابتة (تحفّظي، سقف 50،
  split-only، migrationLog، idempotent).
- **`src/components/DiagnosticsPanel.jsx`:** كارت «المطابقة المالية» بقى فيه
  زرين بدل واحد:
  - **🧮 فحص فقط (آمن)** — `reconcileFinancials({dryRun:true})` (قراءة فقط، زي زمان).
  - **🔧 إصلاح الآن** — `reconcileFinancials({autoRepair:true})` بتأكيد `ask()`،
    بيشغّل الإكمال فورًا وبيعرض عدد الأرجل اللي اتكتبت (بدل انتظار فحص الليل).
  - صندوق النتيجة بيعرض سطر الإصلاح التلقائي (تم/قابل للإصلاح).

### الملفات
`api/cron/reconcile-financials.js` · `src/components/DiagnosticsPanel.jsx` ·
bump (`package.json` + `src/constants/index.js` + `public/sw.js` +
`public/changelog.json`).

build ✓ · 531 اختبار ناجح (الآلية متغطّاة من V185).

> **🔜 متابعة:** بعد ما يثبت في الإنتاج، نضيف صنف رواتب الأسبوع لنفس الحلقة،
> وممكن toggle دائم في الإعدادات يكتب `cfg.reconcile.autoRepair` (محتاج تمرير
> upConfig للوحة التشخيصات — مش موجود حاليًا).

---

## V21.27.185 (2026-06-30) — 🔧 إصلاح تلقائي لأرجل التحويلات الناقصة + تغطية اختبارية للمحرّك المالي

**السياق:** جلسة مراجعة معمارية (فحص شامل للتطبيق). اتحدّد خطران أساسيان:
(1) الكتابة عبر الـ split collections غير ذرّية — `factory/config` بيتحفظ منفصل
عن `*Days`، فلو الأول نجح والتاني فشل، تدفق مالي يتسجّل ناقص لحد ما يتصلح يدويًا.
(2) صفر تغطية اختبارية للمحرّك المالي الحرج.

> **قرار push-back (§0.1):** الحل المباشر لـ (1) = إعادة كتابة `upConfig` لـ
> transaction عابرة للـ collections = **مرفوض** (أخطر دالة، hot path، حد 500
> doc، مفيش بيئة تست). بدّاله: قفل حلقة «الكشف → الإصلاح» server-side خارج الـ
> hot path. التشخيص كشف إن النظام أكثر تحصينًا مما بدا: الـ day-docs ذرّية
> (`runTransaction`)، الفشل بيظهر toast + بيرجّع `{ok:false}` فالـ autoPost
> يتسكِب، والـ reconcile cron بيكتشف الـ drift يوميًا. الفجوة الوحيدة: الإصلاح
> كان **يدوي**.

### 1) إصلاح تلقائي لأرجل التحويلات المؤكدة (`reconcile-financials` cron)
- **موديول مشترك جديد `api/_repairs.js`** — `computeMissingTransferLegs` (نقية،
  صفر I/O) + `applyTransferLegRepairs` (writer، read-modify-write، merge مش
  overwrite، idempotent بالـ id). اتنقل من `repair-confirmed-transfers.js`
  عشان **مصدر حقيقة واحد** يشغّل الـ endpoint اليدوي **و** الـ cron (يمنع drift).
- **الـ cron** بقى بعد الكشف: يحسب الأرجل الناقصة، ولو فيه — يصلح **تلقائيًا**
  بشروط أمان صارمة:
  - **تحفّظي:** يكمّل بس أرجل تحويلات **المستخدم أكّدها** (مفيش اختراع فلوس).
  - **معطّل افتراضيًا:** `cfg.reconcile.autoRepair !== true` → dry-run (يحسب
    ويبلّغ بس). التفعيل = flag واحد، أو `?autoRepair=1` يدوي.
  - **سقف `AUTO_REPAIR_CAP=50`:** انحراف أكبر = عطل منهجي = مراجعة بشرية.
  - **صنف واحد مثبَّت فقط** (أرجل التحويلات)؛ الباقي (رواتب…) alert-only لحد ما
    يثبت في الإنتاج.
  - بس لما الخزنة + التحويلات مقسّمين؛ النطاق = نافذة المطابقة (الأقدم من
    الـ endpoint اليدوي).
  - بيتسجّل في `migrationLog` + بيتذكر في تنبيه الواتساب (شفافية).
- **`repair-confirmed-transfers.js`** اتعاد توصيله للموديول المشترك — سلوك مطابق،
  أقل تكرار.

### 2) تغطية اختبارية للمحرّك المالي الحرج (+24 اختبار → 531 إجمالي)
- **`src/utils/__tests__/splitCollections.test.js`** (13): أول تغطية لمحرّك
  الكتابة المقسّمة (كل حركة خزنة/راتب/شيك بتعدّي عليه) عبر fake Firestore
  in-memory — توجيه/دمج/حذف/تغيير تاريخ/رفض بلا-تاريخ/انتشار الفشل، **+ اختبار
  توصيف بيعيد إنتاج فجوة الكتابة غير الذرّية تنفيذيًا** (baseline لأي إصلاح قادم).
- **`api/__tests__/repairs.test.js`** (11): الدالة النقية `computeMissingTransferLegs`
  بحقن حتمي (idempotency، رجل out/in ناقصة، تخطّي المعلّق/التاريخ الغلط، تحفّظية).

### الملفات
`api/_repairs.js` (جديد) · `api/cron/reconcile-financials.js` ·
`api/maintenance/repair-confirmed-transfers.js` ·
`src/utils/__tests__/splitCollections.test.js` (جديد) ·
`api/__tests__/repairs.test.js` (جديد) · bump (`package.json` +
`src/constants/index.js` + `public/sw.js` + `public/changelog.json`).

**صفر تغيير في سلوك الإنتاج الحالي** (الإصلاح التلقائي معطّل افتراضيًا؛ باقي
التغييرات اختبارات + refactor مطابق). build ✓ · 531 اختبار ناجح.

> **🔜 متابعة:** بعد ما إصلاح أرجل التحويلات يثبت في الإنتاج، نضيف صنف رواتب
> الأسبوع (`repair-payroll-week-treasury`) لنفس حلقة الـ auto-repair. وممكن
> toggle في `DiagnosticsPanel` لتفعيل `cfg.reconcile.autoRepair` من الواجهة.
