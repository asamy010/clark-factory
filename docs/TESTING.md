...
| **٣أ. Emulator smoke** | **يتأكد إن بيئة الاختبار نفسها شغّالة** (auth+firestore+storage round-trip) | `npm run test:emu` | ~دقيقة (آلي) |
| **٣ب. Emulator يدوي** | **التطبيق الحقيقي** + Firestore/Auth/Storage وهميين | `npm run emu` + `npm run dev:emu` | يدوي (كليك) |
| **٤. Staging كلاود** | نسخة حية على لينك preview قبل الإنتاج | push على فرع التطوير | تلقائي (بعد الضبط) |

> **`npm run test:emu`** (V21.27.208): أمر واحد بيشغّل الـ emulator suite ويعمل
> round-trip حقيقي (تسجيل مستخدم على Auth + قراءة موثّقة على القواعد الحقيقية +
> رفع ملف على Storage) ويطلع PASS/FAIL. استخدمه كـ**تأكيد سريع إن بيئة الاختبار
> سليمة** قبل ما تعتمد عليها. (اتّجرّب فعليًا: 3/3 ✅.)
