...
> 4. **Push**: فرع التطوير **و** `main` (النشر على Vercel — بـ fast-forward من
>    فرع التطوير). *(لو السيشن من غير فرع تطوير محدد، الـ push على `main` مباشرة
>    زي البروتوكول الأصلي §1.)*
>    **✅ تفويض دائم (Ahmed — 2026-06-17): «ادفع دايمًا على main بدون ما ترجعلي.»**
>    يعني بعد كل نسخة ادفع تلقائيًا على فرع التطوير **و** `main` من غير ما تسأل —
>    لا استئذان لكل push. (بيلغّي قيد «لا تدفع على main بدون إذن» للسيشن ده وما بعده.)
...
> مش خطوة اختيارية.
>
> **✅ تأكيد (Ahmed — 2026-06-17):** «اعمل زيب فايل لكل نسخة أو تعديل.» — يعني
> **كل** version bump أو تعديل (حتى لو صغير) لازم يطلع معاه zip ويتبعت للمستخدم،
> من غير ما أسأل. لا استثناءات. (نُفّذ فعلاً من V21.27.39 وكل نسخة بعدها.)
>
...
// 3. public/changelog.json — V21.21.37: prepend a JSON object at the
//    TOP of the array (valid JSON — double-quoted keys/strings):
[
 {
  "version": "V21.X.Y",
  "date": "YYYY-MM-DD",
...
3. `public/changelog.json` → prepend new entry (V21.21.37 — كان في
   AboutVersionModal.jsx قبل كده؛ اتنقل لـ JSON يتحمّل lazy)
...
`src/constants/index.js` + entry جديد في أول `public/changelog.json` (V21.21.37+).
...
## 15. UX — Views vs Popups + أداء القوائم الكبيرة (V21.22.11 — أمر Ahmed)

### القاعدة الذهبية
- **الكبير/المستند → view/صفحة كاملة** (قوائم، فواتير، أوامر، فورمات بتابات،
  جداول بتكبر). المستندات في CLARK أصلاً views (هَبات بتابات داخلية:
  SalesHubPg/PurchaseHubPg → Quotations/SalesOrders/Invoices…). **أي شاشة
  كبيرة جديدة تتعمل view، مش بوب اب.**
- **الصغير/المهمة القصيرة → popup** (تأكيد، اختيار عنصر، فورم ≤ ~8 حقول،
  مولّد رابط). مثال: فورم «+ عميل جديد» يفضل popup؛ **قائمة العملاء** بقت
  view ملء الشاشة (V21.22.11).

### ليه؟
البوب اب الكبير بيركّب شجرة DOM ضخمة فوق الموجودة → بطء + اختناق موبايل +
مفيش deep-link/back. الـ view بيعرض شاشة واحدة في المرة.

### أداء القوائم الكبيرة (≥ مئات الصفوف)
1. **Pagination**: اعرض 50 ثم «عرض المزيد» (CustDeliverPg custListLimit
   V21.22.10). متعملش render لـ 1500 صف ثقيل دفعة واحدة.
2. **مفيش حساب per-row في render loop**: أي إجمالي/مانع-حذف/ملخّص لكل عنصر
   لازم يتحسب **مرة واحدة بـ useMemo single-pass** (O(حركات+عناصر))، مش
   لكل عنصر على كل رندر (O(عناصر×حركات)). شوف custTotalsMap/custBlockerMap.
3. **Anti-pattern مميت**: `Map.get(id) || expensiveFallback` — بيتفجّر على
   القيم الصفرية الصالحة (get بترجّع 0 falsy → fallback غالي لكل عنصر صفري).
   استخدم `=== undefined` أو `.has()`. (V21.22.8 — بوب اب العملاء بطّأ مع
   1500 عميل بسبب ده.)
