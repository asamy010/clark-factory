...
/* V21.9.171: like verifyAdminToken but WITHOUT the admin/manager role check.
   Use this for endpoints where any authenticated user is allowed (e.g. push
   notification subscribe — every user subscribes their own device). The
   function still resolves and returns the role so the endpoint can apply
   per-action authorization downstream (e.g. limit broadcast to admins).
   Returns the same shape as verifyAdminToken on success: { ok, uid, email, role }. */
export async function verifyAuthedToken(token) {
  if (!token || typeof token !== "string") {
    return { ok: false, status: 401, error: "رمز المصادقة مطلوب" };
  }
  const clean = token.startsWith("Bearer ") ? token.slice(7).trim() : token.trim();
  ...
  /* Resolve role — same logic as verifyAdminToken but doesn't gate on it. */
  const bootstrapUid = (process.env.BOOTSTRAP_ADMIN_UID || "").trim();
  if (bootstrapUid && decoded.uid === bootstrapUid) {
    return { ok: true, uid: decoded.uid, email: decoded.email, role: "admin" };
  }
  ...
  } catch (_) { /* role stays "viewer" — failing open is fine; downstream checks gate sensitive ops */ }
  return { ok: true, uid: decoded.uid, email: decoded.email, role };
}
