...
export const FEATURE_BUDGETS=[
  ...
  {
    key:"stockMovements",
    label:"📦 حركات المخزن (مقسّمة يومياً)",
    docRef:"stockMovementsDays/*",
    budget:30_000_000,/* 30 MB موزّعة — high-volume warehouse activity */
    getData:(d)=>d?.stockMovements||[],
    getSize:(d)=>_bytes(d?.stockMovements),
    getCount:(d)=>(d?.stockMovements||[]).length,
    advice:"V19.52: مخزّنة في stockMovementsDays — كل حركة مخزن بتاريخ.",
    dataType:"split",
  },
  {
    key:"purchaseReceipts",
    label:"📥 استلامات الشراء (مقسّمة يومياً)",
    docRef:"purchaseReceiptsDays/*",
    budget:10_000_000,
    getData:(d)=>d?.purchaseReceipts||[],
    getSize:(d)=>_bytes(d?.purchaseReceipts),
    getCount:(d)=>(d?.purchaseReceipts||[]).length,
    advice:"V19.52: مخزّنة في purchaseReceiptsDays — كل إيصال استلام لوحده.",
    dataType:"split",
  },
  {
    key:"treasuryTransfers",
    label:"💸 تحويلات الخزنة (مقسّمة يومياً)",
    docRef:"treasuryTransfersDays/*",
    budget:5_000_000,
    getData:(d)=>d?.treasuryTransfers||[],
    getSize:(d)=>_bytes(d?.treasuryTransfers),
    getCount:(d)=>(d?.treasuryTransfers||[]).length,
    advice:"V19.52: مخزّنة في treasuryTransfersDays — تحويلات بين الحسابات.",
    dataType:"split",
  },
  {
    key:"salesAudits",
    label:"📊 جرد المبيعات (مقسّم يومياً)",
    docRef:"salesAuditsDays/*",
    budget:5_000_000,
    getData:(d)=>d?.salesAudits||[],
    getSize:(d)=>_bytes(d?.salesAudits),
    getCount:(d)=>(d?.salesAudits||[]).length,
