...
import { buildDocColumns, sumQtyByUnit, fmtQtyByUnit } from "../docColumns.js";
...
describe("buildDocColumns — الأعمدة الأساسية", () => {
  it("بند بيع: الكود/الاسم/الوحدة + قبل وبعد الخصم لكل صف", () => {
    ...
    expect(totals).toMatchObject({ subBefore: 1000, discount: 100, discountPct: 10, subAfter: 900 });
  });
...
describe("buildDocColumns — نسبة الخصم (V21.21.45)", () => {
  it("نسبة الخصم الفعلية للصف = القيمة ÷ قبل الخصم × 100", () => {
    const { rows, totals } = buildDocColumns([
      { modelNo: "A", qty: 10, unitPrice: 100, discountType: "pct", discountValue: 30 },
    ]);
    expect(rows[0].discountPct).toBe(30);
    expect(totals.discountPct).toBe(30);
  });
  it("خصم رأس موزّع: كل صف نسبته الفعلية = نسبة الرأس", () => {
    const { rows, totals } = buildDocColumns([
      { modelNo: "A", qty: 10, unitPrice: 100 },
      { modelNo: "B", qty: 5, unitPrice: 100 },
    ], { headerDiscountPct: 30 });
    expect(rows[0].discountPct).toBe(30);
    expect(rows[1].discountPct).toBe(30);
    expect(totals.discountPct).toBe(30);
  });
});
...
describe("buildDocColumns — توزيع الخصم الكلي (الضمانة الذهبية)", () => {
  it("خصم رأس % بيتوزّع على الصفوف ومجموع بعد الخصم = الإجمالي", () => {
    ...
    expect(totals).toMatchObject({ subBefore: 1500, discount: 150, discountPct: 10, subAfter: 1350 });
  ...
  it("خصم بنود + خصم رأس معاً: الصف بيجمع الاتنين", () => {
    ...
    expect(totals).toMatchObject({ subBefore: 1000, discount: 190, discountPct: 19, subAfter: 810 });
  });
  ...
  it("بدون خصم: قبل = بعد، وعمود الخصم صفر", () => {
    ...
    expect(totals).toMatchObject({ subBefore: 100, discount: 0, discountPct: 0, subAfter: 100 });
  });
...
/* ── V21.27.107: إجمالي الكمية حسب الوحدة (صف الإجمالي تحت عمود الكمية) ── */
describe("sumQtyByUnit / fmtQtyByUnit", () => {
  it("وحدة واحدة → عنصر واحد بالمجموع", () => {
    const out = sumQtyByUnit([
      { modelNo: "A", qty: 5, unit: "قطعة", unitPrice: 10 },
      { modelNo: "B", qty: 3, unit: "قطعة", unitPrice: 10 },
    ]);
    expect(out).toEqual([{ unit: "قطعة", qty: 8 }]);
    expect(fmtQtyByUnit(out)).toBe("8 قطعة");
  });

  it("وحدات مختلفة → عنصر لكل وحدة + نص مدمج", () => {
    const out = sumQtyByUnit([
      { modelNo: "A", qty: 5, unit: "قطعة", unitPrice: 10 },
      { modelNo: "B", qty: 3, unit: "متر", unitPrice: 10 },
      { modelNo: "C", qty: 2, unit: "قطعة", unitPrice: 10 },
    ]);
    expect(out).toEqual([{ unit: "قطعة", qty: 7 }, { unit: "متر", qty: 3 }]);
    expect(fmtQtyByUnit(out)).toBe("7 قطعة · 3 متر");
  });

  it("بلا وحدة → رقم فقط؛ تتجاهل الأقسام والكميات الصفرية", () => {
    const out = sumQtyByUnit([
      { isSection: true, title: "قسم" },
      { modelNo: "A", qty: 4, unitPrice: 10 },
      { modelNo: "B", qty: 0, unit: "قطعة", unitPrice: 10 },
    ]);
    expect(out).toEqual([{ unit: "", qty: 4 }]);
    expect(fmtQtyByUnit(out)).toBe("4");
  });

  it("buildDocColumns.totals بيرجّع qtyByUnit + totalQty", () => {
    const { totals } = buildDocColumns([
      { modelNo: "A", qty: 5, unit: "قطعة", unitPrice: 10 },
      { modelNo: "B", qty: 3, unit: "متر", unitPrice: 10 },
    ]);
    expect(totals.qtyByUnit).toEqual([{ unit: "قطعة", qty: 5 }, { unit: "متر", qty: 3 }]);
    expect(totals.totalQty).toBe(8);
  });

  it("قائمة فاضية → '0'", () => {
    expect(fmtQtyByUnit(sumQtyByUnit([]))).toBe("0");
  });
});
