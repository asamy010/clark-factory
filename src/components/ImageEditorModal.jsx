...
export function ImageEditorModal({ src, logoUrl, data, prefill, onClose, onSave }){
  ...
  const addText = () => {
    const id = nid();
    /* V21.27.32: الافتراضيات الجديدة (طلب Ahmed) — خط Anton، لون أسود،
       بدون بولد، بدون ظل. */
    setLayers(ls => [...ls, { id, type: "text", cx: dims.w / 2, cy: dims.h / 2, rot: 0, opacity: 1,
      text: "اكتب هنا", font: "'Anton', sans-serif", size: Math.max(24, Math.round(dims.h * 0.07)), color: "#000000",
      bold: false, italic: false, align: "center", shadow: false, stroke: false, strokeColor: "#000000" }]);
    setSelId(id);
  };
  ...
  const buildBlob = async () => {
    ...
    if(document.fonts && document.fonts.load){
      const want = new Set();
      for(const L of layers){ if(L.type === "text") want.add((L.bold ? "700 " : "400 ") + "48px " + L.font); }
