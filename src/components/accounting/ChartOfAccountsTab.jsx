...
/* V19.50: pre-upgrade CoA backup goes to backups/ collection, not into
   factory/config (which used to bloat the doc by ~12 KB per upgrade). */
...
export function ChartOfAccountsTab({coa, allEntries, upConfig, T, FS, isMob, showToast, userName}){
  ...
  const upgradeToLatest = async () => {
    ...
    /* V19.50: Write pre-upgrade backup to backups/ collection BEFORE the
       upConfig mutation (separate doc — does NOT bloat factory/config).
       If the backup write fails, abort the upgrade so we never apply changes
       without a recoverable copy. The previous behavior dumped the backup as
       an inline key on factory/config which leaked ~12 KB into the main doc
       on every CoA upgrade — V19.50 migration also cleans those up. */
    const ts = new Date().toISOString().replace(/[:.]/g, "-");
    try {
      await fsSetDoc(fsDoc(db, "backups", "coa-pre-upgrade-" + ts), {
        label: "CoA backup قبل ترقية شجرة الحسابات",
        autoGenerated: true,
        backupType: "coa-pre-upgrade",
        createdAt: new Date().toISOString(),
        coaCountBefore: (coa || []).length,
        accountsAdded: analysis.missing.length,
        coa: JSON.parse(JSON.stringify(coa || [])),
      });
    } catch (backupErr) {
      console.error("[V19.50] Failed to write CoA pre-upgrade backup — aborting upgrade:", backupErr);
      await tell("تعذر الحفظ", "تعذر إنشاء النسخة الاحتياطية لشجرة الحسابات. تم إلغاء الترقية كي لا يتم التعديل بدون نسخة قابلة للاسترداد.\n\nالخطأ: " + (backupErr?.message || String(backupErr)).slice(0, 200), { danger: true });
      return;
    }

    /* Apply the upgrade in a single config mutation (no inline backup key) */
