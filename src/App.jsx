...
export default function App(){
  ...
  /* V18.95: Notification expiry duration. Values: "1h"|"2h"|"1d"|"endday"|"none". Default: "2h". */
  const[qpDuration,setQpDuration]=useState("2h");
  /* V18.95 HOTFIX: notifTick state must live BEFORE any early returns to keep hook order stable across renders */
  const[_notifTick,setNotifTick]=useState(0);
  /* V18.95: Toggle for the "all notifications" popup that opens when user clicks "+N more" chip */
  const[notifPopupOpen,setNotifPopupOpen]=useState(false);
  /* V18.95 HOTFIX: ticker effect also must run unconditionally (no early-return skip).
  ...
  /* V18.95: Filter notifications honoring expiresAt + endedAt + dismissedBy.
  ...
  const userNotifs=(config.notifications||[]).filter(n=>n.toEmail===userEmail||n.toEmail==="all").filter(n=>{
    ...
    /* V18.95: forAdminsOnly notifs (e.g. transfer approval requests) only show for admins */
  ...
  /* V18.95: Urgent tasks bar in topbar disabled — these now show in the greeting bar
  ...
  /* V18.95: End-for-everyone — sender or admin clicks ⏹ → endedAt set → hidden for all users.
  ...
  /* V18.95: Notifications shown in sub-bar — all types (تذكير/طلب/مهمة/مهمة عاجلة).
  ...
  /* V18.95: Notification link handler — clicking a chip with `link` field navigates
  ...
  const handleNotifLinkClick=(n)=>{
    ...
    if(type==="invoice"){
      ...
      /* V18.95: Sub-type "transfer_pending" → opens transfers view in TreasuryPg */
  ...
  /* V18.95: Live ticker is wired at top of component (before early returns) for hook-order stability. */
  ...
  return<div onClick={()=>{if(showAlerts)setShowAlerts(false);if(gSearch)setGSearch("");if(showLogout)setShowLogout(false)}} style={{minHeight:"100vh",direction:"rtl",fontFamily:"'Cairo',sans-serif",background:T.bg,color:T.text,fontSize:FS,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{padding:isMob?"8px 12px":"10px 24px",background:T.navBg||T.cardSolid,borderBottom:T.navBg?"none":"1px solid "+T.brd,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0,gap:isMob?8:16,minHeight:isMob?52:60}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:isMob?6:10,flexShrink:0}}>
        ...
        {!isMob&&<div style={{display:"flex",alignItems:"center",gap:6,lineHeight:1}}>
          ...
          >V18.95 <span style={{fontSize:FS-3,opacity:0.7}}>📋</span></span>
        </div>}
        {isMob&&<>
      ...
      <div style={{display:"flex",alignItems:"center",gap:isMob?4:6,flexShrink:0}}>
        ...
        {/* V18.95: Season badge — moved here from greeting bar to keep greeting-bar single-row.
    ...
    <div style={{flex:1,overflow:"auto",padding:isMob?"8px 10px":"12px 24px"}}>
      ...
      {tab==="home"&&(()=>{
        ...
        return<div>
          ...
          {/* ═══ GREETING HEADER — minimal & elegant — V18.95: notif chips in middle ═══ */}
          <div className="home-greet" style={{padding:isMob?"14px 16px":"18px 24px",borderRadius:16,marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
            ...
            {/* V18.95: Notification chips — bigger size, single-row (no wrap), "+N more" button when overflowing.
            ...
            {/* V18.95: Season badge moved to top bar (next to bell). Removed from here to keep
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:16,padding:20,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        {quickPopup==="task"?<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
            ...
            {/* V18.95: Display duration — sender chooses how long the notification stays visible */}
          ...
          <Btn primary onClick={()=>{if(!qpText.trim())return;const to=qpTo||"all";const targetUser=targets.find(u=>u.email===to);
            /* V18.95: Compute expiresAt based on selected duration. */
            let expiresAt=null;
            const now=new Date();
    ...
    {/* V18.95: Full notifications popup — opens when user clicks "+N more" chip in greeting bar */}
    ...
    <AboutVersionModal open={showAboutVersion} onClose={()=>setShowAboutVersion(false)} currentVersion="V18.95"/>
  </div>
}
