  ...
  return<div style={{position:"fixed",inset:0,background:"#000",zIndex:99999,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
      ...
      :<>
        ...
        <div style={{position:"absolute",width:220,height:220,border:"3px solid #10B981",borderRadius:16,boxShadow:"0 0 0 9999px rgba(0,0,0,0.4)"}}/>
        <div style={{position:"absolute",bottom:30,left:"50%",transform:"translateX(-50%)",color:"#fff",fontSize:14,fontWeight:600,background:"rgba(0,0,0,0.6)",padding:"8px 20px",borderRadius:10}}>وجّه الكاميرا على كود QR</div>
      </>}
    </div>
  </div>
}
...
export default function App(){
  ...
  const[tab,setTab]=useState("home");const[sel,setSel]=useState(null);const[gSearch,setGSearch]=useState("");const[showAlerts,setShowAlerts]=useState(false);const[showLogout,setShowLogout]=useState(false);const[showScanner,setShowScanner]=useState(false);
  ...
  const appAlerts=(()=>{try{const a=[];
    data.orders.forEach(o=>{const wds=o.workshopDeliveries||[];const pieces=o.orderPieces||[];
      ...
      if(pieces.length>0){const linkedPieces=new Set();FKEYS.forEach(k=>{if(gf(o,k))(o["fabricPieces"+k]||[]).forEach(p=>linkedPieces.add(p))});const unlinked=pieces.filter(p=>!linkedPieces.has(p));unlinked.forEach(p=>a.push({msg:o.modelNo+" — متبقي "+p+" «لم يتم القص»",color:T.purple,icon:"🧵",orderId:o.id}))}
      /* Pieces in production but not delivered to any workshop */
      if(o.status==="في التشغيل"&&pieces.length>0){const t=calcOrder(o);pieces.forEach(p=>{const delivered=wds.some(wd=>wd.garmentType===p);if(!delivered)a.push({msg:o.modelNo+" — "+p+" ("+t.cutQty+" قطعة) متاح للتسليم والتشغيل",color:T.warn,icon:"🏭",orderId:o.id})})}
    });
    /* Delay alerts */
  ...
  return<div onClick={()=>{if(showAlerts)setShowAlerts(false);if(gSearch)setGSearch("");if(showLogout)setShowLogout(false)}} style={{minHeight:"100vh",direction:"rtl",fontFamily:"'Cairo',sans-serif",background:T.bg,color:T.text,fontSize:FS,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{padding:isMob?"8px 10px":"12px 28px",background:T.cardSolid,borderBottom:"1px solid "+T.brd,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:isMob?6:10}}>
        {/* QR Scanner */}
        <div onClick={()=>setShowScanner(true)} style={{cursor:"pointer",fontSize:isMob?16:20,padding:"2px 6px",borderRadius:8,background:T.accentBg}}>📷</div>
        {/* Alerts Bell */}
  ...
  return<div>
    <Card title={"الانتاج - الموسم "+season+" ("+orders.length+" موديل)"} style={{marginBottom:12}}>
      <div style={{display:"grid",gridTemplateColumns:isMob?"1fr 1fr":"repeat(6,1fr)",gap:10}}>
        ...
        <div style={{padding:10,borderRadius:8,background:"#8B5CF606",border:"1px solid #8B5CF612",textAlign:"center"}}><div style={{fontSize:FS-2,color:T.textSec}}>عند الورش</div><div style={{fontSize:isMob?18:22,fontWeight:800,color:"#8B5CF6"}}>{fmt(Math.max(0,inProdQty))}</div><div style={{fontSize:FS-3,color:T.textMut}}>{"تسليم: "+fmt(totalDeliveredToWs)+" | استلام: "+fmt(totalReceivedFromWs)}</div></div>
...
function DBPg({data,upConfig,isMob,canEdit,statusCards}){
  ...
  const[gName,setGName]=useState("");const[gEid,setGEid]=useState(null);const[gIconSel,setGIconSel]=useState("👕");
  ...
  return<div>
    ...
    {sub==="size"&&<Card title="المقاسات">{canEdit&&<div style={{display:"grid",gridTemplateColumns:isMob?"1fr":"3fr 1fr auto",gap:10,marginBottom:16}}>
      <Inp value={sfld.label} onChange={v=>setSfld({...sfld,label:v})} placeholder="المقاسات (مثال: S-M-L-XL)"/>
      <Inp type="number" value={sfld.pcs||""} onChange={v=>setSfld({...sfld,pcs:Number(v)||0})} placeholder="قطع/سيري"/>
      <div style={{display:"flex",gap:4}}><Btn primary onClick={saveSize}>{sfld._eid?"تحديث":"+ اضافة"}</Btn>{sfld._eid&&<Btn ghost onClick={()=>setSfld({label:"",pcs:0,_eid:null})}>الغاء</Btn>}</div>
    </div>}<table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["#","المقاسات","قطع/سيري",...(canEdit?[""]:[])] .map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead><tbody>{data.sizeSets.map((s,i)=><tr key={s.id} style={{background:sfld._eid===s.id?T.warn+"10":"transparent"}}><td style={TD}>{i+1}</td><td style={{...TD,fontWeight:600}}>{s.label}</td><td style={{...TDB,color:T.accent}}>{s.pcsPerSeries||"-"}</td>{canEdit&&<td style={{...TD,whiteSpace:"nowrap"}}><div style={{display:"flex",gap:4}}>{eBtn(()=>setSfld({label:s.label,pcs:s.pcsPerSeries||0,_eid:s.id}))}<DelBtn onConfirm={()=>upConfig(d=>{d.sizeSets=d.sizeSets.filter(x=>x.id!==s.id)})} blocked={sizeBlock(s)}/></div></td>}</tr>)}</tbody></table></Card>}
    {sub==="garment"&&<Card title="قطع الموديل">{canEdit&&<div style={{display:"grid",gridTemplateColumns:"auto 3fr auto",gap:8,marginBottom:16,alignItems:"end"}}>
      <div><label style={{fontSize:FS-2,color:T.textSec,whiteSpace:"nowrap"}}>الأيقونة</label><Sel value={gIconSel} onChange={setGIconSel}>{GARMENT_ICONS.map(ic=><option key={ic} value={ic}>{ic}</option>)}</Sel></div>
      <div><label style={{fontSize:FS-2,color:T.textSec,whiteSpace:"nowrap"}}>اسم القطعة</label><Inp value={gName} onChange={setGName} placeholder="مثال: قميص، شورت، تيشيرت"/></div>
      <div style={{display:"flex",gap:4}}><Btn primary onClick={saveGarment}>{gEid?"تحديث":"+ اضافة"}</Btn>{gEid&&<Btn ghost onClick={()=>{setGName("");setGEid(null);setGIconSel("👕")}}>الغاء</Btn>}</div>
    </div>}
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>{(data.garmentTypes||[]).map(g=><span key={g.id} style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 18px",borderRadius:12,border:"1px solid "+(gEid===g.id?T.warn:T.brd),fontSize:FS,fontWeight:600,background:gEid===g.id?T.warn+"10":T.cardSolid}}>{(g.icon||gIcon(g.name,data.garmentTypes))+" "+g.name}{canEdit&&<>{" "}{eBtn(()=>{setGName(g.name);setGEid(g.id);setGIconSel(g.icon||gIcon(g.name,data.garmentTypes))})}<DelBtn onConfirm={()=>upConfig(d=>{d.garmentTypes=(d.garmentTypes||[]).filter(x=>x.id!==g.id)})} blocked={garmentBlock(g)}/></>}</span>)}</div>
...
function OrdPg({data,addOrder,delOrder,updOrder,goD,isMob,canEdit,statusCards,upConfig}){
  ...
  const handleBarcode=(v)=>{const q=v.trim();if(!q)return;const ord=data.orders.find(o=>o.modelNo===q);if(ord){goD(ord.id)}else{setOQ(q);setShowFilter(true)}};
  return<div>
    {/* Barcode Scanner Bar */}
    <div style={{marginBottom:10,display:"flex",gap:6,alignItems:"center"}}>
      <div style={{flex:1,position:"relative"}}>
        <input ref={barcodeRef} placeholder="📷 مسح باركود أو ادخل رقم الموديل..." onKeyDown={e=>{if(e.key==="Enter"){handleBarcode(e.target.value);e.target.value=""}}} style={{width:"100%",padding:"10px 14px",borderRadius:10,border:"2px solid "+T.accent+"40",fontSize:FS+1,fontFamily:"inherit",background:T.cardSolid,color:T.text,boxSizing:"border-box",outline:"none"}}/>
      </div>
    </div>
