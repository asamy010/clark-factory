...
function renderLabelPages(d,n){
  ...
  for(let i=1;i<=n;i++){h+="<div class='pg'><div class='brand'>CLARK Factory</div><div class='tp'>"+d.arrow+" "+d.title+"</div>"
    ...
    +"<div class='bot'>"+(d.qrSrc?"<img src='"+d.qrSrc+"'/>":"")+(n>1?"<div class='bags'>"+i+"/"+n+"</div>":"")+"</div>"
...
export default function App(){
  ...
  return<div onClick={()=>{if(showAlerts)setShowAlerts(false);if(gSearch)setGSearch("");if(showLogout)setShowLogout(false)}} style={{minHeight:"100vh",direction:"rtl",fontFamily:"'Cairo',sans-serif",background:T.bg,color:T.text,fontSize:FS,display:"flex",flexDirection:"column"}}>
    ...
    {labelPopup&&<div className="pop-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setLabelPopup(null)}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:24,width:"100%",maxWidth:320,border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>
          <Btn ghost onClick={()=>setLabelPopup(null)}>الغاء</Btn>
          <Btn onClick={()=>{renderLabelPages(labelPopup,labelBags);setLabelPopup(null);setLabelBags(1)}} style={{background:T.accent,color:"#fff",border:"none",fontWeight:700}}>{"🖨 طباعة "+labelBags}</Btn>
        </div>
      </div>
    </div>}
    ...
    {barcodePopup&&(()=>{const allOrders=data.orders||[];const ps=data.printSettings||{};const lw=ps.labelWidth||50;const lh=ps.labelHeight||40;const mg=ps.margins||2;const fl=ps.fields||{};
      ...
      const printQR=()=>{if(!selOrder){showToast("⚠️ اختر موديل");return}const qrText="CLARK:"+selOrder.id+":"+rs;
        const qrMM=Math.min(lw-mg*2,lh-mg*2)-8;/* QR fills most of the label */
        let h="";for(let i=0;i<copies;i++){h+="<div class='lbl'>";
          ...
          if(fl.modelNo?.show!==false)h+="<div style='font-weight:800;font-size:"+((fl.modelNo?.size||16)/2.5)+"mm;line-height:1.1'>"+selOrder.modelNo+"</div>";
          if(fl.desc?.show)h+="<div style='font-size:"+((fl.desc?.size||10)/2.5)+"mm;color:#444;line-height:1'>"+(selOrder.modelDesc||"")+"</div>";
          ...
          if(fl.series?.show!==false)h+="<div style='font-weight:700;font-size:"+((fl.series?.size||12)/2.5)+"mm;line-height:1'>سيري: "+rs+"</div>";
          if(fl.sizeLabel?.show)h+="<div style='font-size:"+((fl.sizeLabel?.size||10)/2.5)+"mm;line-height:1'>"+(selOrder.sizeLabel||"—")+"</div>";
          if(fl.price?.show)h+="<div style='font-size:"+((fl.price?.size||10)/2.5)+"mm;line-height:1'>"+(selOrder.sellPrice||"—")+" ج.م</div>";
          h+="</div>"}
        const w=window.open("","_blank");w.document.write("<html dir='rtl'><head><title>QR</title><script src='https://cdn.jsdelivr.net/npm/qrcode/build/qrcode.min.js'></"+"script><style>@page{size:"+lw+"mm "+lh+"mm;margin:"+mg+"mm}*{margin:0;padding:0}body{margin:0;padding:0;font-family:'Cairo',Arial,sans-serif}.lbl{width:"+(lw-mg*2)+"mm;height:"+(lh-mg*2)+"mm;page-break-after:always;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;overflow:hidden}</style></head><body>"+h+"<script>document.querySelectorAll('.qr-img').forEach(img=>{QRCode.toDataURL(img.dataset.text,{width:400,margin:0}).then(url=>{img.src=url}).catch(()=>{})});setTimeout(()=>window.print(),800)</"+"script></body></html>");w.document.close();
...
function CustDeliverPg({data,upConfig,updOrder,isMob,isTab,canEdit,user}){
  ...
  const delSession=(sessId)=>{const sess=sessions.find(s=>s.id===sessId);if(!sess)return;
    ...
    affectedOrders.forEach(orderId=>{updOrder(orderId,o=>{
      ...
      o.customerReturns=(o.customerReturns||[]).filter(r=>r.sessId!==sessId)})});
  ...
  const getCustTotal=(custId)=>orders.reduce((s,o)=>{const del=(o.customerDeliveries||[]).filter(d=>d.custId===custId).reduce((ss,d)=>ss+(Number(d.qty)||0),0);const ret=(o.customerReturns||[]).filter(r=>r.custId===custId).reduce((ss,r)=>ss+(Number(r.qty)||0),0);return s+del-ret},0);
  ...
  const updateSessStatus=(sessId,status)=>{upConfig(d=>{const si=(d.custDeliverySessions||[]).findIndex(s=>s.id===sessId);if(si>=0)d.custDeliverySessions[si].status=status})};
  ...
  const printShippingLabel=async(cust,sessDate,items,total,shipN)=>{
    let qrSrc="";try{const QR=await loadQR();if(QR)qrSrc=await QR.toDataURL(window.location.origin+"?cust="+encodeURIComponent(cust.name),{width:160,margin:1})}catch(e){}
    const pw=window.open("","_blank");if(!pw)return;
    let pages="";for(let i=1;i<=shipN;i++){
      pages+="<div class='pg'><div class='from'><b>CLARK Factory</b></div><div class='to'><div class='tn'>"+cust.name+"</div><div class='tp'>"+cust.phone+"</div>"+(cust.address?"<div class='ta'>"+cust.address+"</div>":"")+"</div>"
      +"<table>";items.forEach(it=>{if(it.qty>0)pages+="<tr><td>"+it.no+"</td><td>"+it.qty+"</td></tr>"});
      pages+="<tr class='tt'><td>الاجمالي</td><td>"+total+"</td></tr></table>"
      +"<div class='bb'>"+(qrSrc?"<img src='"+qrSrc+"' class='qr'/>":"")+"<div class='sn'>"+i+"/"+shipN+"</div></div>"
      +"<div class='dt'>"+sessDate+"</div></div>"+(i<shipN?"<div style='page-break-after:always'></div>":"")
    }
    ...
    +".from{text-align:center;font-size:11pt;padding:2mm;border-bottom:2px solid #000;margin-bottom:2mm}"
    +".to{text-align:center;padding:3mm;border:3px solid #000;border-radius:8px;margin-bottom:3mm}"
    +".tn{font-size:18pt;font-weight:800}.tp{font-size:11pt}.ta{font-size:9pt;color:#444}"
    +"table{width:100%;border-collapse:collapse;margin:2mm 0}td{padding:1.5mm 3mm;border:1px solid #000;font-weight:700;font-size:10pt}"
    +".tt td{background:#eee;font-weight:800;font-size:11pt}"
    +".bb{display:flex;align-items:center;justify-content:center;gap:5mm;margin:auto 0;padding:3mm 0}"
    +".qr{width:22mm;height:22mm}.sn{font-size:30pt;font-weight:800;border:3px solid #000;border-radius:8px;padding:2mm 6mm}"
    +".dt{text-align:center;font-size:8pt;color:#555;border-top:1px dashed #000;padding-top:1mm}"
  ...
  return<div>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,width:"100%",maxWidth:isMob?"100%":window.innerWidth-48,maxHeight:"92vh",border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:isMob?"12px 16px":"16px 24px",borderBottom:"1px solid "+T.brd,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:4}}><Btn small onClick={()=>printSession(activeSess.id)} style={{background:T.accentBg,color:T.accent,border:"1px solid "+T.accent+"30"}} title="طباعة">🖨</Btn><Btn ghost small onClick={()=>closeMatrix()} title="إغلاق">✕</Btn></div>
          </div>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",minWidth:aMods.length*90+180}}>
          ...
          <tbody>
              ...
              return<tr key={c.id} style={{background:ci%2===0?"transparent":T.bg+"80"}}>
                ...
                {aMods.map((m,mi)=>{const k=m.id+"_"+c.id;const q=Number(aGrid[k])||0;const isEd=editCell===k;
                  return<td key={m.id} style={{...TD,textAlign:"center",padding:2,cursor:canEdit?"pointer":"default",background:isEd?T.warn+"10":q>0?T.ok+"04":"transparent"}}
                    ...
                    {isEd?<input type="number" autoFocus value={editVal}
                ...
                <td style={{...TD,whiteSpace:"nowrap",padding:"2px 4px"}}>{rowTotal>0&&<div style={{display:"flex",gap:2}}>
                  ...
                  <Btn small onClick={()=>{setShipPopup({cust:c,total:rowTotal});setShipCount(1)}} style={{background:"#F59E0B12",color:"#F59E0B",border:"1px solid #F59E0B30",fontSize:9,padding:"2px 5px"}} title="طباعة ليبل حراري">🏷️</Btn>
                  <Btn small onClick={()=>{const items=aMods.map(m=>({no:m.modelNo,qty:Number(aGrid[m.id+"_"+c.id])||0})).filter(x=>x.qty>0);printShippingLabel(c,activeSess.date,items,rowTotal,1)}} style={{background:"#6366F112",color:"#6366F1",border:"1px solid #6366F130",fontSize:9,padding:"2px 5px"}}>📮</Btn>
            ...
            <tr><td style={{...TD,fontWeight:800,color:T.warn}}>رصيد حالي</td>
              ...
              <td style={TD}></td><td style={TD}></td></tr>
            {canEdit&&<tr><td style={{...TD,fontWeight:700,color:"#8B5CF6"}}>💰 سعر البيع</td>
    ...
    {(()=>{const totalStock=stockModels.reduce((s,m)=>s+m.stockQty,0);const totalSold=stockModels.reduce((s,m)=>s+m.custDel,0);const totalRemain=totalStock-totalSold;const pct=totalStock?Math.round(totalSold/totalStock*100):0;
      ...
      return<div style={{marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMob?"repeat(2,1fr)":"repeat(4,1fr)",gap:isMob?8:12,marginBottom:14}}>
          <div style={{padding:12,borderRadius:12,background:T.accent+"08",border:"1px solid "+T.accent+"15",textAlign:"center"}}><div style={{fontSize:FS-2,color:T.textSec}}>استلام مخزن جاهز</div><div style={{fontSize:isMob?18:24,fontWeight:800,color:T.accent}}>{fmt(totalStock)}</div></div>
          <div style={{padding:12,borderRadius:12,background:T.ok+"08",border:"1px solid "+T.ok+"15",textAlign:"center"}}><div style={{fontSize:FS-2,color:T.textSec}}>تسليم عملاء</div><div style={{fontSize:isMob?18:24,fontWeight:800,color:T.ok}}>{fmt(totalSold)}</div><div style={{fontSize:FS-3,color:T.ok}}>{pct+"%"}</div></div>
          <div style={{padding:12,borderRadius:12,background:T.warn+"08",border:"1px solid "+T.warn+"15",textAlign:"center"}}><div style={{fontSize:FS-2,color:T.textSec}}>رصيد حالي</div><div style={{fontSize:isMob?18:24,fontWeight:800,color:T.warn}}>{fmt(totalRemain)}</div></div>
          <div style={{padding:12,borderRadius:12,background:"#8B5CF608",border:"1px solid #8B5CF615",textAlign:"center"}}><div style={{fontSize:FS-2,color:T.textSec}}>نسبة مبيعات / الرصيد</div><div style={{fontSize:isMob?18:24,fontWeight:800,color:"#8B5CF6"}}>{pct+"%"}</div><div style={{fontSize:FS-3,color:T.textMut}}>{fmt(totalSold)+" / "+fmt(totalStock)}</div></div>
        </div>
      </div>})()}
    ...
    {salesAnalysis&&(()=>{const topCusts=[...customers].map(c=>({...c,total:getCustTotal(c.id)})).filter(c=>c.total>0).sort((a,b)=>b.total-a.total);const maxCust=topCusts[0]?.total||1;
      return<div className="pop-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:isMob?8:16}} onClick={()=>setSalesAnalysis(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:isMob?16:24,width:"100%",maxWidth:isMob?"100%":window.innerWidth-48,maxHeight:"90vh",overflowY:"auto",border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
          ...
          {topCusts.length>0?<div>
            <div style={{overflowX:"auto",marginBottom:16}}>
              <div style={{display:"flex",alignItems:"flex-end",gap:isMob?8:14,minHeight:200,padding:"10px 0"}}>
                {topCusts.map((c,i)=>{const pc=maxCust?Math.round(c.total/maxCust*100):0;const maxH=160;
                  return<div key={c.id} onClick={()=>{const details=[];orders.forEach(o=>{const cd=(o.customerDeliveries||[]).filter(d=>d.custId===c.id).reduce((s,d)=>s+(Number(d.qty)||0),0);if(cd>0)details.push({name:o.modelNo+" — "+o.modelDesc,qty:cd})});setSalesDetail({title:"📦 تفاصيل "+c.name,items:details,total:c.total,color:"#0EA5E9"})}} title={c.name+" — "+fmt(c.total)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,cursor:"pointer",flex:1,minWidth:isMob?50:70}}>
                    <div style={{fontSize:isMob?FS-2:FS,fontWeight:800,color:i===0?"#F59E0B":T.accent}}>{fmt(c.total)}</div>
                    <div style={{width:"100%",maxWidth:isMob?32:50,height:Math.max(10,pc/100*maxH),borderRadius:"8px 8px 0 0",background:i===0?"linear-gradient(180deg,#F59E0B,#EAB308)":"linear-gradient(180deg,#0EA5E9,#0284C7)",transition:"height 0.5s"}}/>
                    <div style={{fontSize:isMob?FS-3:FS-1,fontWeight:700,color:T.text,textAlign:"center",lineHeight:1.2,maxWidth:isMob?60:100,wordBreak:"break-word"}}>{c.name}</div>
                  </div>})}
              </div>
            </div>
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["#","العميل","النوع","تسليم","مرتجع","صافي"].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead><tbody>
              {topCusts.map((c,i)=>{const ret=orders.reduce((s,o)=>(o.customerReturns||[]).filter(r=>r.custId===c.id).reduce((ss,r)=>ss+(Number(r.qty)||0),s),0);return<tr key={c.id} style={{background:i%2===0?"transparent":T.bg+"80"}}><td style={TD}>{i+1}</td><td style={{...TD,fontWeight:700}}>{c.name}</td><td style={{...TD,fontSize:FS-2,color:T.textMut}}>{c.type||"—"}</td><td style={{...TD,textAlign:"center",fontWeight:700,color:T.accent}}>{fmt(c.total+ret)}</td><td style={{...TD,textAlign:"center",color:ret?T.err:T.textMut}}>{ret||"—"}</td><td style={{...TD,textAlign:"center",fontWeight:800}}>{fmt(c.total)}</td></tr>})}
            </tbody></table></div>
    ...
    <Card title={"📦 سجل التسليمات ("+sessions.length+")"}>
      ...
      {(()=>{const fSess=sortedSessions.filter(s=>{if(!sessFilterQ.trim())return true;const q=sessFilterQ.trim().toLowerCase();const mNos=s.modelIds.map(id=>{const o=orders.find(x=>x.id===id);return o?.modelNo||""}).join(" ").toLowerCase();const cNames=s.custIds.map(id=>{const c=customers.find(x=>x.id===id);return c?.name||""}).join(" ").toLowerCase();return(s.date||"").includes(q)||mNos.includes(q)||cNames.includes(q)});
        ...
        {fSess.map(s=>{const totalQty=Object.values(s.grid||{}).reduce((sum,v)=>sum+(Number(v)||0),0);const isActive=activeSession===s.id;const st=s.status||"جاري التجهيز";const stColor=st==="تم التسليم"?"#10B981":st==="تم الشحن"?"#0EA5E9":"#F59E0B";
          const confirmed=s.saleConfirmed;const isFree=s.freeSale;
          return<div key={s.id} style={{padding:"12px 16px",borderRadius:12,background:isActive?T.accent+"08":T.cardSolid,border:isActive?"2px solid "+T.accent:confirmed?"1px solid #10B98130":"1px solid "+T.brd,cursor:"pointer",transition:"all 0.15s"}} onClick={()=>setActiveSession(isActive?null:s.id)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:18}}>{isFree?"🔓":"📦"}</span>
                <div><div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span style={{fontWeight:700,fontSize:FS,color:T.text}}>{(isFree?"بيع حر":"سجل توزيع")+" — "+s.date}</span><span style={{fontSize:FS-3,fontWeight:700,color:stColor,background:stColor+"15",padding:"1px 8px",borderRadius:6,border:"1px solid "+stColor+"30"}}>{st}</span>
    ...
    {qrSale&&(()=>{const isSale=qrSale.mode==="sale";const title=isSale?"📦 بيع سريع":"↩️ مرتجع سريع";const color=isSale?"#10B981":"#8B5CF6";
      ...
      const handleScan=(text)=>{try{
        ...
        try{const j=JSON.parse(text);if(j.app==="clark"&&j.type==="pkg"){const pkg=(config.packages||[]).find(p=>p.id===j.id);
          ...
          if(isSale){const newItems=[];let blocked=false;pkg.items.forEach(it=>{const o=orders.find(x=>x.id===it.orderId);if(!o)return;
            const avail=getAvailStock(it.orderId);const alreadyInCart=(actualByModel[it.orderId]||0);
        ...
        /* Regular model QR */
        const parts=text.split(":");if(parts[0]!=="CLARK"||parts.length<3)return;const orderId=parts[1];const rs=Number(parts[2])||1;
        const o=orders.find(x=>x.id===orderId);if(!o){playBeep("error");showToast("⛔ موديل غير موجود");return}
        if(isSale){/* Check stock */
          const alreadyInCart=(actualByModel[orderId]||0);const avail=getAvailStock(orderId);
      ...
      if(!qrSale.custId)return<div className="pop-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={closeQrSale}>
        <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:24,width:"100%",maxWidth:450,maxHeight:"80vh",overflowY:"auto",border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontSize:FS+2,fontWeight:800,color}}>{title+" — اختر العميل"}</div>
            <Btn ghost small onClick={closeQrSale} title="إغلاق">✕</Btn>
          </div>
          <div style={{marginBottom:10}}><Inp value={custFilter} onChange={setCustFilter} placeholder="بحث بالاسم أو التليفون..."/></div>
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            {customers.filter(c=>{if(!custFilter.trim())return true;const q=custFilter.trim().toLowerCase();return(c.name||"").toLowerCase().includes(q)||(c.phone||"").includes(q)}).map(c=><div key={c.id} onClick={()=>{setQrSale(p=>({...p,custId:c.id,linkedSession:isSale?undefined:"free"}));setCustFilter("")}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:10,cursor:"pointer",border:"1px solid "+T.brd}} onMouseEnter={e=>e.currentTarget.style.background=color+"08"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              <span style={{fontWeight:700}}>{c.name}</span>
              <span style={{fontSize:FS-1,color:T.accent}}>{"صافي: "+getCustTotal(c.id)}</span>
            </div>)}
          </div>
        </div>
      </div>;
      if(isSale&&qrSale.linkedSession===undefined){const custSessions=sessions.filter(s=>{const hasGrid=Object.keys(s.grid||{}).some(k=>k.endsWith("_"+qrSale.custId)&&Number(s.grid[k])>0);return hasGrid&&!s.saleConfirmed});
        return<div className="pop-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={closeQrSale}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:24,width:"100%",maxWidth:500,maxHeight:"80vh",overflowY:"auto",border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div><div style={{fontSize:FS+2,fontWeight:800,color}}>📦 اختر طريقة البيع</div><div style={{fontSize:FS-1,color:T.textMut}}>{customers.find(c=>c.id===qrSale.custId)?.name}</div></div>
              <div style={{display:"flex",gap:4}}><Btn ghost small onClick={()=>setQrSale(p=>({...p,custId:null}))}>← رجوع</Btn><Btn ghost small onClick={closeQrSale}>✕</Btn></div>
            </div>
            {custSessions.length>0&&<div style={{marginBottom:12}}>
              <div style={{fontSize:FS,fontWeight:700,color:T.text,marginBottom:6}}>📋 بيع مربوط بسجل توزيع:</div>
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {custSessions.map(s=>{const totalQ=Object.entries(s.grid||{}).filter(([k])=>k.endsWith("_"+qrSale.custId)).reduce((sum,[,v])=>sum+(Number(v)||0),0);
                  return<div key={s.id} onClick={()=>setQrSale(p=>({...p,linkedSession:s.id}))} style={{padding:"10px 14px",borderRadius:10,cursor:"pointer",border:"1px solid "+T.accent+"30",background:T.accent+"04"}} onMouseEnter={e=>e.currentTarget.style.background=T.accent+"10"} onMouseLeave={e=>e.currentTarget.style.background=T.accent+"04"}>
                    ...
                    <div style={{fontSize:FS-2,color:T.textMut}}>{s.modelIds?.length+" موديل"}</div>
                  </div>})}
              </div>
            </div>}
            <div onClick={()=>setQrSale(p=>({...p,linkedSession:"free"}))} style={{padding:14,borderRadius:12,border:"1px solid "+color+"30",background:color+"06",cursor:"pointer",textAlign:"center"}} onMouseEnter={e=>e.currentTarget.style.background=color+"12"} onMouseLeave={e=>e.currentTarget.style.background=color+"06"}>
      ...
      return<div className="pop-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:isMob?8:16}} onClick={closeQrSale}>
        <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:isMob?16:24,width:"100%",maxWidth:isMob?420:600,maxHeight:"90vh",overflowY:"auto",border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:4}}><Btn ghost small onClick={()=>setQrSale(p=>({...p,linkedSession:isSale?undefined:"free",items:[]}))}>{isSale?"← سجل":"← عميل"}</Btn><Btn ghost small onClick={closeQrSale}>✕</Btn></div>
          </div>
          {linkedSess&&qrSale.items.length>0&&(()=>{const allModels=[...new Set([...Object.keys(plannedByModel),...Object.keys(actualByModel)])];
            return<div style={{border:"1px solid "+T.brd,borderRadius:10,overflow:"hidden",marginBottom:10}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><th style={{...TH,fontSize:FS-2}}>الموديل</th><th style={{...TH,fontSize:FS-2}}>المخطط</th><th style={{...TH,fontSize:FS-2}}>الفعلي</th><th style={{...TH,fontSize:FS-2}}>الحالة</th></tr></thead><tbody>
                {allModels.map(oid=>{const o=orders.find(x=>x.id===oid);const planned=plannedByModel[oid]||0;const actual=actualByModel[oid]||0;const diff=actual-planned;
                  return<tr key={oid} style={{background:diff===0?"#10B98108":diff>0?"#F59E0B08":"#EF444408"}}><td style={{...TD,fontWeight:700,color:T.accent}}>{o?.modelNo||"?"}</td><td style={{...TD,textAlign:"center"}}>{planned}</td><td style={{...TD,textAlign:"center",fontWeight:800}}>{actual}</td>
                    <td style={{...TD,textAlign:"center",fontWeight:700,color:diff===0?"#10B981":diff>0?"#F59E0B":"#EF4444"}}>{diff===0?"✅ تمام":diff>0?"⚠️ +"+diff:"❌ "+diff}</td></tr>})}
              </tbody></table>
            </div>})()}
          {qrScanActive?<div style={{marginBottom:12}}>
            ...
            <div style={{textAlign:"center",marginTop:8}}><Btn small onClick={()=>{setQrScanActive(false);try{const v=document.getElementById("qr-sale-video");if(v&&v.srcObject){v.srcObject.getTracks().forEach(t=>t.stop());v.srcObject=null}}catch(e){}}} style={{background:T.err+"12",color:T.err,border:"1px solid "+T.err+"30"}}>⏹ إيقاف</Btn></div></div>
          :<div style={{textAlign:"center",marginBottom:12}}><Btn onClick={()=>{setQrScanActive(true);setTimeout(()=>{const startCam=async()=>{try{const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment",width:{ideal:640}}});
            ...
            const hasBD=typeof BarcodeDetector!=="undefined";const detector=hasBD?new BarcodeDetector({formats:["qr_code"]}):null;let lastScan="";let lastTime=0;
            const scan=async()=>{if(!v.srcObject)return;const c=document.getElementById("qr-sale-canvas");if(!c||v.readyState<2){requestAnimationFrame(scan);return}
              ...
              if(detector){try{const codes=await detector.detect(c);if(codes.length>0){const t=codes[0].rawValue;const now=Date.now();if(t!==lastScan||now-lastTime>2000){lastScan=t;lastTime=now;handleScan(t)}}}catch(e){}}
          ...
          <div style={{marginBottom:12}}><SearchSel value="" onChange={v=>{if(!v)return;const rs=getRackSize(v);handleScan("CLARK:"+v+":"+rs)}} options={stockModels.filter(m=>m.avail>0).map(m=>({value:m.id,label:m.modelNo+" — "+m.modelDesc+" ("+m.avail+")"}))} placeholder="اختر موديل..."/></div>
    ...
    {pkgPopup&&pkgPopup.startsWith("view_")&&(()=>{const pkgId=pkgPopup.replace("view_","");const pkg=(config.packages||[]).find(p=>p.id===pkgId);if(!pkg)return null;
      ...
      const addToPkg=(orderId)=>{const o=orders.find(x=>x.id===orderId);if(!o)return;const rs=getRackSize(orderId);
        upConfig(d=>{const pi=d.packages.findIndex(p=>p.id===pkgId);if(pi<0)return;
      ...
      const updatePkgItem=(idx,newCount)=>{upConfig(d=>{const pi=d.packages.findIndex(p=>p.id===pkgId);if(pi<0)return;const it=d.packages[pi].items[idx];if(!it)return;
      ...
      const removePkgItem=(idx)=>{upConfig(d=>{const pi=d.packages.findIndex(p=>p.id===pkgId);if(pi<0)return;const it=d.packages[pi].items[idx];
        if(!d.packages[pi].movements)d.packages[pi].movements=[];
      ...
      const printContents=()=>{const qrData=JSON.stringify({app:"clark",type:"pkg",id:pkg.id,num:pkg.number});const mvs=pkg.movements||[];
        const pw=window.open("","_blank");let h="<html dir='rtl'><head><meta charset='utf-8'/><title>"+pkg.number+"</title><script src='https://cdn.jsdelivr.net/npm/qrcode/build/qrcode.min.js'></"+"script><link href='https://fonts.googleapis.com/css2?family=Cairo:wght@600;800;900&display=swap' rel='stylesheet'/><style>@page{margin:10mm}*{box-sizing:border-box}body{font-family:'Cairo',sans-serif;margin:0;padding:0;color:#000}@media print{.no-print{display:none}}</style></head><body style='padding:10mm'>";
        h+="<div style='text-align:center;margin-bottom:8mm'><canvas id='qr'></canvas></div>";
        h+="<div style='text-align:center;margin-bottom:6mm'><div style='font-size:28pt;font-weight:900;letter-spacing:3px'>CLARK</div><div style='font-size:22pt;font-weight:800;color:#0EA5E9;margin:2mm 0'>📦 "+pkg.number+"</div><div style='font-size:14pt;color:#444'>"+pkg.date+(pkg.note?" — "+pkg.note:"")+"</div></div>";
        h+="<table style='width:100%;border-collapse:collapse;margin-bottom:6mm;font-size:14pt'><thead><tr style='background:#EFF6FF'><th style='border:2px solid #000;padding:3mm;font-weight:900'>الموديل</th><th style='border:2px solid #000;padding:3mm;font-weight:900'>سيريهات</th><th style='border:2px solid #000;padding:3mm;font-weight:900'>الكمية</th></tr></thead><tbody>";
        (pkg.items||[]).forEach(it=>{h+="<tr><td style='border:1px solid #000;padding:3mm;font-weight:800;font-size:16pt;text-align:center'>"+it.modelNo+"</td><td style='border:1px solid #000;padding:3mm;text-align:center;font-size:14pt'>"+it.count+"</td><td style='border:1px solid #000;padding:3mm;text-align:center;font-weight:900;font-size:18pt;color:#0EA5E9'>"+it.qty+"</td></tr>"});
        h+="<tr style='background:#EFF6FF'><td style='border:2px solid #000;padding:3mm;font-weight:900;font-size:14pt'>الاجمالي</td><td style='border:2px solid #000;padding:3mm;text-align:center;font-weight:800;font-size:14pt'>"+totalSeries+"</td><td style='border:2px solid #000;padding:3mm;text-align:center;font-weight:900;font-size:22pt;color:#0EA5E9'>"+totalQ+"</td></tr></tbody></table>";
        if(mvs.length>0){h+="<div style='margin-top:6mm;font-size:12pt'><div style='font-weight:900;font-size:14pt;margin-bottom:3mm;border-bottom:2px solid #000;padding-bottom:2mm'>سجل الحركات</div>";
          mvs.forEach(m=>{const icon=m.type==="add"?"📥 اضافة":"📤 سحب";const color=m.type==="add"?"#10B981":"#EF4444";
            h+="<div style='padding:2mm 0;border-bottom:1px solid #eee;display:flex;gap:4mm'><span>"+m.date+"</span><span style='color:"+color+";font-weight:800'>"+icon+"</span><span style='font-weight:700'>"+m.modelNo+"</span><span>× "+m.count+"</span><span style='font-weight:800'>("+m.qty+" ق)</span>"+(m.by?"<span style='color:#888'>"+m.by+"</span>":"")+"</div>"});
          h+="<div style='margin-top:3mm;font-weight:900;font-size:16pt;color:#0EA5E9'>الرصيد الحالي: "+totalQ+" قطعة</div></div>"}
        h+="<script>QRCode.toCanvas(document.getElementById('qr'),'"+qrData.replace(/'/g,"\\'")+"',{width:250,margin:1},()=>{});setTimeout(()=>window.print(),800)</"+"script></body></html>";
    ...
    {pkgAction&&(pkgAction.mode==="add"||pkgAction.mode==="remove")&&(()=>{const pkg=(config.packages||[]).find(p=>p.id===pkgAction.id);if(!pkg)return null;
      ...
      const handlePkgScan=(text)=>{try{const parts=text.split(":");if(parts[0]!=="CLARK"||!parts[1])return;const orderId=parts[1];const o=orders.find(x=>x.id===orderId);if(!o){playBeep("error");showToast("⛔ موديل غير موجود");return}
        ...
        if(isAdd){upConfig(d=>{const pi=d.packages.findIndex(p=>p.id===pkgAction.id);if(pi<0)return;
          const existing=d.packages[pi].items.findIndex(it=>it.orderId===orderId);
          if(existing>=0){d.packages[pi].items[existing].count++;d.packages[pi].items[existing].qty=d.packages[pi].items[existing].count*d.packages[pi].items[existing].rackSize}
          else{d.packages[pi].items.push({orderId,modelNo:o.modelNo,rackSize:rs,count:1,qty:rs})}
          if(!d.packages[pi].movements)d.packages[pi].movements=[];
        ...
        else{upConfig(d=>{const pi=d.packages.findIndex(p=>p.id===pkgAction.id);if(pi<0)return;
          const existing=d.packages[pi].items.findIndex(it=>it.orderId===orderId);
        ...
        <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:isMob?16:24,width:"100%",maxWidth:isMob?420:500,maxHeight:"90vh",overflowY:"auto",border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
          ...
          <div style={{position:"relative",width:"100%",maxWidth:280,margin:"0 auto 12px",borderRadius:12,overflow:"hidden",background:"#000"}}>
            <video id="pkg-action-video" playsInline muted autoPlay style={{width:"100%",display:"block"}} ref={el=>{if(!el||el.srcObject)return;(async()=>{try{const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment",width:{ideal:640}}});el.srcObject=stream;
              const hasBD=typeof BarcodeDetector!=="undefined";const detector=hasBD?new BarcodeDetector({formats:["qr_code"]}):null;const canvas=document.createElement("canvas");let lastScan="";let lastTime=0;
              const scan=async()=>{if(!el.srcObject)return;if(el.readyState<2){requestAnimationFrame(scan);return}canvas.width=el.videoWidth;canvas.height=el.videoHeight;canvas.getContext("2d").drawImage(el,0,0);
                if(detector){try{const codes=await detector.detect(canvas);if(codes.length>0){const t=codes[0].rawValue;const now=Date.now();if(t!==lastScan||now-lastTime>2000){lastScan=t;lastTime=now;handlePkgScan(t)}}}catch(e){}}
    ...
    {custQR&&<div className="pop-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setCustQR(null)}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:20,padding:24,width:"100%",maxWidth:320,border:"1px solid "+T.brd,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",textAlign:"center"}}>
        ...
        <div style={{marginTop:12}}><Btn onClick={()=>{const pw=window.open("","_blank");if(!pw)return;pw.document.write("<!DOCTYPE html><html><head><style>body{display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:Cairo,sans-serif;flex-direction:column}@media print{button{display:none}}</style></head><body><h2>"+custQR.name+"</h2><p>"+custQR.phone+"</p><img src='"+custQR.src+"' style='width:250px'/><br/><button onclick='window.print()' style='margin-top:20px;padding:8px 24px;font-size:16px'>🖨 طباعة</button></body></html>");pw.document.close()}} style={{background:T.accentBg,color:T.accent,border:"1px solid "+T.accent+"30"}}>🖨 طباعة QR</Btn></div>
      </div>
    </div>}
  </div>
}
...
function SettingsPg({config,upConfig,isMob,user,theme,setTheme,season,orders,syncWsIds,replaceOrder,updOrder}){
  ...
  return<div>
    ...
    <Card title="🖨 إعدادات طباعة QR" style={{marginBottom:16}}>
      {(()=>{const ps=config.printSettings||{labelWidth:50,labelHeight:40,orientation:"portrait",margins:2,fields:{brand:{show:true,size:14},modelNo:{show:true,size:16},desc:{show:true,size:10},qr:{show:true,size:80},series:{show:true,size:12},sizeLabel:{show:false,size:10},price:{show:false,size:10}}};
        ...
        const printTest=()=>{const w=ps.labelWidth||50;const h=ps.labelHeight||40;const m=ps.margins||2;const orient=ps.orientation||"portrait";
          const pw_=window.open("","_blank");let html="<html dir='rtl'><head><title>طباعة تجريبية</title><script src='https://cdn.jsdelivr.net/npm/qrcode/build/qrcode.min.js'></"+"script><style>@page{size:"+w+"mm "+h+"mm;margin:"+m+"mm}body{margin:0;padding:0;font-family:'Cairo',Arial,sans-serif}.lbl{width:"+(w-m*2)+"mm;height:"+(h-m*2)+"mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}</style></head><body><div class='lbl'>";
          if(ps.fields?.brand?.show)html+="<div style='font-weight:900;font-size:"+((ps.fields?.brand?.size||14)/2.5)+"mm;letter-spacing:2px'>CLARK</div>";
          if(ps.fields?.modelNo?.show)html+="<div style='font-weight:800;font-size:"+((ps.fields?.modelNo?.size||16)/2.5)+"mm'>3262114</div>";
          if(ps.fields?.desc?.show)html+="<div style='font-size:"+((ps.fields?.desc?.size||10)/2.5)+"mm;color:#444'>توينز اولادي قطعتين</div>";
          if(ps.fields?.qr?.show)html+="<canvas id='qr' style='margin:2mm 0'></canvas>";
          if(ps.fields?.series?.show)html+="<div style='font-weight:700;font-size:"+((ps.fields?.series?.size||12)/2.5)+"mm'>سيري: 4 قطعة</div>";
          if(ps.fields?.sizeLabel?.show)html+="<div style='font-size:"+((ps.fields?.sizeLabel?.size||10)/2.5)+"mm'>مقاس: 6-8-10-12</div>";
          if(ps.fields?.price?.show)html+="<div style='font-size:"+((ps.fields?.price?.size||10)/2.5)+"mm'>السعر: 95 ج.م</div>";
          html+="</div><script>if(document.getElementById('qr'))QRCode.toCanvas(document.getElementById('qr'),'CLARK:test:4',{width:"+(ps.fields?.qr?.size||80)+",margin:1},()=>{});setTimeout(()=>window.print(),800)</"+"script></body></html>";
        ...
        return<div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{fontSize:FS,fontWeight:700,color:T.text}}>محتوى الليبل:</div>
          {fields.map(f=>{const fv=ps.fields?.[f.key]||{show:false,size:12};return<div key={f.key} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0"}}>
            ...
            {fv.show&&f.key!=="qr"&&<div style={{width:60}}><Inp type="number" value={fv.size||12} onChange={v=>savePS(s=>{if(!s.fields)s.fields={};if(!s.fields[f.key])s.fields[f.key]={show:true,size:12};s.fields[f.key].size=Number(v)||12})}/></div>}
            {fv.show&&f.key==="qr"&&<div style={{width:60}}><Inp type="number" value={fv.size||80} onChange={v=>savePS(s=>{if(!s.fields)s.fields={};if(!s.fields[f.key])s.fields[f.key]={show:true,size:80};s.fields[f.key].size=Number(v)||80})}/></div>}
          </div>})}
          <div style={{display:"flex",gap:8}}><Btn onClick={printTest} style={{background:T.accent+"12",color:T.accent,border:"1px solid "+T.accent+"30"}}>🖨 طباعة تجريبية</Btn></div>
        </div>})()}
    </Card>
    {/* Data Maintenance */}
    <Card title="🔧 صيانة البيانات" style={{marginBottom:16}}>
      {(()=>{
        ...
        orders.forEach(o=>{const t=calcOrder(o);
          ...
          (o.customerReturns||[]).forEach(r=>{if(r.sessId&&!(config.custDeliverySessions||[]).some(s=>s.id===r.sessId))issues.push({ord:o.id,no:o.modelNo,msg:"مرتجع يتيم (جلسة محذوفة)",sev:"err"})});
        });
        ...
        const cleanOrphans=()=>{
          ...
          orders.forEach(o=>{const hasBadDel=(o.customerDeliveries||[]).some(d=>d.sessionId&&!sessIds.has(d.sessionId));const hasBadRet=(o.customerReturns||[]).some(r=>r.sessId&&!sessIds.has(r.sessId));
            if(hasBadDel||hasBadRet)updOrder(o.id,u=>{u.customerDeliveries=(u.customerDeliveries||[]).filter(d=>!d.sessionId||sessIds.has(d.sessionId));u.customerReturns=(u.customerReturns||[]).filter(r=>!r.sessId||sessIds.has(r.sessId))})});
          /* Clean orphan grid entries in sessions & audits */
          upConfig(d=>{(d.custDeliverySessions||[]).forEach(s=>{if(!s.grid)return;Object.keys(s.grid).forEach(k=>{const[oid,cid]=k.split("_");if(!orderIds.has(oid)||!custIds.has(cid))delete s.grid[k]})});
