...
/* V19.26: Bulk messaging / campaigns engine */
...
/* V19.26: Removed TeamActivityModal import — feature retired (topbar pill cleanup) */
...
export default function App(){
  ...
  /* V19.26: Notification expiry duration. Values: "1h"|"2h"|"1d"|"endday"|"none". Default: "2h". */
  const[qpDuration,setQpDuration]=useState("2h");
  /* V19.26 HOTFIX: notifTick state must live BEFORE any early returns to keep hook order stable across renders */
  const[_notifTick,setNotifTick]=useState(0);
  /* V19.26: Toggle for the "all notifications" popup that opens when user clicks "+N more" chip */
  const[notifPopupOpen,setNotifPopupOpen]=useState(false);
  /* V19.26 HOTFIX: ticker effect also must run unconditionally (no early-return skip).
  ...
  /* V19.26: Online-only mode — block ALL writes when offline. isOnlineRef gives callbacks
  ...
  /* V19.26: Last sync timestamp — updated by upConfigTx/upSalesTx/upTasksTx on success.
  ...
  /* V19.26: human-friendly relative time used for "آخر مزامنة من ..." pill and the team panel rows. */
  ...
  /* V19.26: Force re-render every 30s so the relative-time display ("من X ثانية") stays fresh. */
  const[,setSyncTick]=useState(0);
  useEffect(()=>{const t=setInterval(()=>setSyncTick(x=>x+1),30000);return()=>clearInterval(t)},[]);
  /* V19.26: Removed showTeamActivity state — feature retired */
  ...
  useEffect(()=>{if(!user)return;
    ...
    const rebuild=()=>{
      ...
      const flatten=(map,pendingMap)=>{
        ...
        /* V19.26 FIX: Track duplicate ids across day docs for diagnostic logging.
        ...
        for(const dayKey of sortedDays){
          ...
          for(const e of entries){
            ...
            /* V19.26 FIX: skip if already added from a newer day doc */
        ...
        /* V19.26: Surface duplicates once per session for diagnostics */
        if(dupIds.size>0){
          ...
          for(const id of dupIds){
            if(!window.__clarkSeenDups.has(id)){
              ...
              console.warn("[V19.26 DEDUP] Duplicate id "+id+" found across day docs — kept newest, hiding older copies. The cleanup migration will remove duplicates from Firestore.");
            }
          }
        }
  ...
  const upConfigTx=useCallback(async(precomputedNext,precomputedNewSplit,precomputedNewPart,explicitSplitBefore,explicitPartBefore)=>{
    ...
    for(let attempt=0;attempt<5;attempt++){
      try{
        ...
        /* V19.26: write reached the server — record the sync timestamp for the topbar pill. */
        markSynced();
        /* V16.74: sync split day docs */
        if(splitActive&&splitAfter){
          /* V19.26 FIX: Retry sync up to 3 times with backoff. The previous
          ...
          for(let syncAttempt=0;syncAttempt<3;syncAttempt++){
            try{
              ...
              syncErr=e;
              console.warn("[V19.26] syncAllSplitChanges attempt "+(syncAttempt+1)+" failed:",e?.message||e);
        ...
        if(partActive&&partAfter){
          /* V19.26 FIX: same retry pattern for hrWeeks partitioned writes */
          let syncErr=null;
          for(let syncAttempt=0;syncAttempt<3;syncAttempt++){
            try{
              ...
              syncErr=e;
              console.warn("[V19.26] syncAllPartitionedChanges attempt "+(syncAttempt+1)+" failed:",e?.message||e);
              if(syncAttempt<2)await _sleep(150*Math.pow(2,syncAttempt));
            }
          }
          if(syncErr){
  ...
  const upConfig=useCallback(fn=>{
    /* V19.26: Online-only mode — refuse all writes when device is offline.
    ...
    if(!isOnlineRef.current){
      console.warn("[V19.26] Refusing upConfig — device is offline");
  ...
  const upSales=useCallback(fn=>{
    /* V19.26: Online-only — refuse writes when offline. Same enforcement as upConfig. */
    if(!isOnlineRef.current){
      showToast("⛔ أنت أوفلاين دلوقتي — التعديل مش متاح لحد ما النت يرجع");
      return;
    }
  ...
  const upTasks=useCallback(fn=>{
    /* V19.26: Online-only — refuse writes when offline. */
    if(!isOnlineRef.current){
      showToast("⛔ أنت أوفلاين دلوقتي — التعديل مش متاح لحد ما النت يرجع");
      return;
    }
  ...
  /* V19.26: Filter notifications honoring expiresAt + endedAt + dismissedBy.
  ...
  const userNotifs=(config.notifications||[]).filter(n=>n.toEmail===userEmail||n.toEmail==="all").filter(n=>{
    ...
    /* V19.26: forAdminsOnly notifs (e.g. transfer approval requests) only show for admins */
  ...
  /* V19.26: Urgent tasks bar in topbar disabled — these now show in the greeting bar
  ...
  /* V19.26: End-for-everyone — sender or admin clicks ⏹ → endedAt set → hidden for all users.
  ...
  /* V19.26: Notifications shown in sub-bar — all types (تذكير/طلب/مهمة/مهمة عاجلة).
  ...
  /* V19.26: Notification link handler — clicking a chip with `link` field navigates
  ...
  const handleNotifLinkClick=(n)=>{
    ...
    if(type==="invoice"){
      ...
      /* V19.26: Sub-type "transfer_pending" → opens transfers view in TreasuryPg */
  ...
  /* V19.26: Live ticker is wired at top of component (before early returns) for hook-order stability. */
  ...
  return<div onClick={()=>{if(showAlerts)setShowAlerts(false);if(gSearch)setGSearch("");if(showLogout)setShowLogout(false)}} style={{minHeight:"100vh",direction:"rtl",fontFamily:"'Cairo',sans-serif",background:T.bg,color:T.text,fontSize:FS,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{padding:isMob?"8px 12px":"10px 24px",background:T.navBg||T.cardSolid,borderBottom:T.navBg?"none":"1px solid "+T.brd,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0,gap:isMob?8:16,minHeight:isMob?52:60}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:isMob?6:10,flexShrink:0}}>
        ...
        {!isMob&&<div style={{display:"flex",alignItems:"center",gap:6,lineHeight:1}}>
          ...
          {/* V19.26: removed "مزامنة من X د" timestamp pill + "👥 الفريق" pill — too noisy in topbar */}
          <span 
          ...
          >V19.26 <span style={{fontSize:FS-3,opacity:0.7}}>📋</span></span>
        </div>}
        {isMob&&<>
      ...
      <div style={{display:"flex",alignItems:"center",gap:isMob?4:6,flexShrink:0}}>
        ...
        {/* V19.26: Season badge — moved here from greeting bar to keep greeting-bar single-row.
    ...
    {/* V19.26: Read-only banner — appears under the topbar whenever the device
    ...
    <div style={{flex:1,overflow:"auto",padding:isMob?"8px 10px":"12px 24px"}}>
      ...
      {tab==="home"&&(()=>{
        ...
        return<div>
          ...
          {/* ═══ GREETING HEADER — V19.26: single-row guaranteed, chips shrink instead of wrapping ═══ */}
          <div className="home-greet" style={{padding:isMob?"14px 16px":"18px 24px",borderRadius:16,marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"nowrap",gap:12,minWidth:0}}>
            ...
            {/* V19.26: Chips compress (shrink) instead of wrapping when space gets tight.
            ...
            {/* V19.26: Season badge moved to top bar (next to bell). Removed from here to keep
          ...
          {!isMob?<div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:18,alignItems:"flex-start",maxWidth:1400,margin:"0 auto"}}>
            ...
            <div>
              {/* V19.26: Tile width capped to ~130px (was filling the column = ~160-180px),
      ...
      {tab!=="home"&&canViewTab(tab)&&<div>
        ...
        {/* V19.26: Bulk messaging campaigns */}
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.cardSolid,borderRadius:16,padding:20,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        {quickPopup==="task"?<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
            ...
            {/* V19.26: Display duration — sender chooses how long the notification stays visible */}
          ...
          <Btn primary onClick={()=>{if(!qpText.trim())return;const to=qpTo||"all";const targetUser=targets.find(u=>u.email===to);
            /* V19.26: Compute expiresAt based on selected duration. */
            let expiresAt=null;
            const now=new Date();
    ...
    {/* V19.26: Full notifications popup — opens when user clicks "+N more" chip in greeting bar */}
    ...
    <AboutVersionModal open={showAboutVersion} onClose={()=>setShowAboutVersion(false)} currentVersion="V19.26"/>
    {/* V19.26: Removed <TeamActivityModal/> render — feature retired */}
  </div>
}
