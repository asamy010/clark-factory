...
export default function App(){
  ...
  /* V21.27.140: عتبة أوسع للهوم بس (التابلت/الآيباد لحد 1300px) — الآيباد
     الحديث عرضه ~1194 فكان بياخد لياوت الديسكتوب الـ4 أعمدة بدل «الأزرار فوق
     والقوائم تحت». مش بنغيّر isTab العام (٧ صفحات بتعتمد عليه — blast radius). */
  const isTabHome=w>=768&&w<1300;
    ...
    <div style={{flex:1,overflowY:"auto",overflowX:isMob?"hidden":"auto",padding:isMob?"8px 10px":"12px 24px",paddingBottom:isMob?"calc(95px + env(safe-area-inset-bottom, 0px))":"12px",paddingTop:isMob&&currentSubViews.length>1&&tab!=="moreMenu"?64:(isMob?8:12),width:"100%",maxWidth:"100vw",boxSizing:"border-box"}}>
        ...
        return<div>
          ...
          <div className="home-greet" style={{padding:isMob?"8px 14px":"10px 20px",borderRadius:14,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"nowrap",gap:12,minWidth:0}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0,flexWrap:"wrap",justifyContent:"flex-end"}}>
              {/* V21.27.103: زر «محافظ إلكترونية» اتشال من الهوم (طلب Ahmed) —
                  موجود أصلاً في الخزنة (TreasuryPg → تاب المحافظ). */}
          ...
          {!isMob?<div style={{display:"grid",gridTemplateColumns:isTabHome?"repeat(6,minmax(0,1fr))":"20fr 50fr 15fr 15fr",gap:isTabHome?14:20,alignItems:"flex-start",...(isTabHome?{width:"100%"}:{maxWidth:1700,margin:"0 auto"})}}>
            ...
            {/* ═══════════════════════════════════════════════════════════
                ...
                zero notifications.
                ═══════════════════════════════════════════════════════════ */}
            <div style={{position:isTabHome?"static":"sticky",top:12,...(isTabHome?{gridColumn:"1 / 3",gridRow:2}:{})}}>
            ...
            {/* ═══════════════════════════════════════════════════════════
                ...
                ═══════════════════════════════════════════════════════════ */}
            <div style={isTabHome?{gridColumn:"1 / 7",gridRow:1}:undefined}>
              <div style={{textAlign:"center"}}>
                <div style={{display:isTabHome?"block":"inline-block",textAlign:"start"}}>
                  <div style={{display:"grid",gridTemplateColumns:isTabHome?"repeat(auto-fill, minmax(120px, 1fr))":"repeat(6, minmax(0, 105px))",gap:isTabHome?14:24}}>
                    {visibleTabs.map(t=>{const perm=getTabPerm(t.key);
                      ...
                      return<div key={t.key} onClick={()=>goTo(t.key)} className="home-tile" style={{background:T.cardSolid,borderRadius:11,padding:"6px 4px",border:"1px solid "+T.brd,textAlign:"center",opacity:perm==="view"?0.75:1,position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:5,aspectRatio:"1"}}>
                        ...
                        <div style={{fontSize:labelFs,fontWeight:800,color:T.text,lineHeight:1.15,width:"100%",maxWidth:"100%",...(isTabHome?{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}:{whiteSpace:"nowrap"})}} title={t.label}>{t.label}</div>
            ...
            {/* ═══════════════════════════════════════════════════════════
                ...
                ═══════════════════════════════════════════════════════════ */}
            <div style={{position:isTabHome?"static":"sticky",top:12,...(isTabHome?{gridColumn:"3 / 5",gridRow:2}:{})}}>
            ...
            {/* ═══════════════════════════════════════════════════════════
                ...
                ═══════════════════════════════════════════════════════════ */}
            <div style={{position:isTabHome?"static":"sticky",top:12,...(isTabHome?{gridColumn:"5 / 7",gridRow:2}:{})}}>
      ...
      {tab!=="home"&&canViewPage(tab)&&<div>
        ...
        {tab==="details"&&<div>
          ...
          {(detView==="orders"||sel)&&<DetPg data={data} updOrder={updOrder} replaceOrder={replaceOrder} addOrder={addOrder} delOrder={delOrder} sel={sel} setSel={setSel} isMob={isMob} isTab={isTab} isTabHome={isTabHome} canEdit={canEditTab("details")} canEditWarehouse={canEditTab("warehouse")} statusCards={statusCards} goHome={goHome} upConfig={upConfig} user={user}/>}
